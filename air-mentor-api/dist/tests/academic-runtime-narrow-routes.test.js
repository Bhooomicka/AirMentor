...
function buildRuntimeChangeSet(scope) {
...
async function applyRuntimeChanges(app, cookie, mode, changeSet) {
    const finalTask = {
        ...changeSet.task,
    ...
    if (mode === 'narrow') {
        ...
        expect(taskListResponse.json().items.some((item) => item.id === changeSet.task.id && item.version === 2)).toBe(true);
        ...
        expect(placementListResponse.json().items.some((item) => item.taskId === changeSet.placement.taskId)).toBe(true);
        ...
        expect(calendarListResponse.json().items.some((item) => item.id === changeSet.auditEvent.id)).toBe(true);
        return;
    }
...
async function readRuntimeProjection(app, cookie, changeSet) {
    ...
    return {
        task: bootstrap.runtime.tasks.find((item) => item.id === changeSet.task.id) ?? null,
        placement: bootstrap.runtime.taskPlacements[changeSet.placement.taskId] ?? null,
        auditEvent: bootstrap.runtime.calendarAudit.find((item) => item.id === changeSet.auditEvent.id) ?? null,
    };
}
describe('academic runtime narrow routes', () => {
    ...
    it('leaves the legacy runtime shadow untouched when per-entity writes persist authoritative rows', async () => {
        ...
        const taskPayload = JSON.parse(tasksRow?.payloadJson ?? '[]');
        const placementPayload = JSON.parse(placementsRow?.payloadJson ?? '{}');
        const auditPayload = JSON.parse(auditRow?.payloadJson ?? '[]');
        ...
        expect(response.statusCode).toBe(200);
        const bootstrap = response.json();
        expect(bootstrap.runtime.tasks.some((item) => item.id === changeSet.task.id)).toBe(true);
        expect(bootstrap.runtime.taskPlacements[changeSet.placement.taskId]).toMatchObject({
            taskId: changeSet.placement.taskId,
        });
        expect(bootstrap.runtime.calendarAudit.some((item) => item.id === changeSet.auditEvent.id)).toBe(true);
    });
    it('ignores shadow-only runtime extras once authoritative task, placement, and calendar rows exist', async () => {
        ...
        const bootstrap = response.json();
        expect(bootstrap.runtime.tasks.some((item) => item.id === changeSet.task.id)).toBe(true);
        expect(bootstrap.runtime.tasks.some((item) => item.id === shadowTaskId)).toBe(false);
        expect(bootstrap.runtime.taskPlacements[changeSet.placement.taskId]).toMatchObject({
            taskId: changeSet.placement.taskId,
        });
        expect(bootstrap.runtime.taskPlacements[shadowTaskId]).toBeUndefined();
        expect(bootstrap.runtime.calendarAudit.some((item) => item.id === changeSet.auditEvent.id)).toBe(true);
        expect(bootstrap.runtime.calendarAudit.some((item) => item.id === shadowAuditEvent.id)).toBe(false);
    });
});
