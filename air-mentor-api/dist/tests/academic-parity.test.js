...
import { academicTerms, academicAssets, courses, facultyOfferingOwnerships, mentorAssignments, sectionOfferings, simulationRuns, simulationStageCheckpoints, studentEnrollments, } from '../src/db/schema.js';
...
async function _grantCourseOwnership(cookie, offeringId, facultyId = 't1') {
    if (!current)
...
async function loadAcademicBootstrap(cookie, simulationStageCheckpointId) {
    if (!current)
        throw new Error('Test app is not initialized');
    const response = await current.app.inject({
        method: 'GET',
...
function collectLeafComponentDefs(nodes) {
    const leafDefs = [];
    const visit = (items) => {
...
describe('academic bootstrap', () => {
    it('keeps faculty-profile proof context and linked proof drilldowns aligned for the active teaching role', async () => {
        ...
        if (!firstQueueStudentId)
            throw new Error('Expected a checkpoint-bound faculty queue student');
    ...
    it('exposes non-overlapping faculty timetable blocks in the proof bootstrap for course-leader playback', async () => {
        ...
        if (!timetable)
            throw new Error('Expected a proof timetable for mnc_t1');
        const classBlocks = timetable.classBlocks;
    ...
    it('ignores legacy academic asset snapshots and derives the live view from admin-owned records', async () => {
        ...
        const proofFaculty = snapshot.faculty.find((faculty) => faculty.facultyId === proofFacultyId);
        ...
        expect(snapshot.faculty.some((faculty) => faculty.facultyId === 'legacy-faculty')).toBe(false);
        expect(snapshot.offerings.some((offering) => offering.offId === 'legacy-offering')).toBe(false);
        ...
        if (!firstStudent)
            throw new Error('Expected a proof-scoped bootstrap student');
    ...
    it('reflects admin master-data changes into the academic bootstrap on the next fetch', async () => {
        ...
        if (!targetOffering)
            throw new Error('Expected a proof-scoped offering for the master-data refresh test');
        ...
        if (!targetOfferingRow)
            throw new Error('Expected a persisted offering row');
        ...
        if (!currentCourse)
            throw new Error('Expected a persisted course row');
        ...
        expect(snapshot.offerings.find((offering) => offering.offId === targetOffering.offId)?.title).toBe(updatedTitle);
        expect(snapshot.faculty.find((faculty) => faculty.facultyId === facultyId)?.offeringIds).toContain(targetOffering.offId);
    });
    it('persists resolved course outcomes, offering schemes, and question papers through backend-owned routes', async () => {
        ...
        expect(targetOffering).toBeTruthy();
        if (!targetOffering)
            throw new Error('Expected a proof-scoped offering for the curriculum persistence test');
        ...
        if (!offeringRow)
            throw new Error('Expected a persisted proof offering row');
    ...
    it('persists authoritative queue, calendar workspace, attendance, and TT1 entry state through the teaching routes', async () => {
        ...
        expect(targetOffering).toBeTruthy();
        if (!targetOffering)
            throw new Error('Expected a proof-scoped offering for the teaching state test');
        ...
        if (!targetStudent)
            throw new Error('Expected a proof-scoped student for the teaching state test');
        ...
        const refreshedStudent = finalBootstrap.studentsByOffering[targetOffering.offId].find((student) => student.id === targetStudent.id);
    ...
    it('keeps faculty-profile proof payloads and student drilldowns scoped for course leaders and mentors', async () => {
        ...
        const [selectedCheckpoint] = await current.db.select().from(simulationStageCheckpoints).where(eq(simulationStageCheckpoints.simulationRunId, activeRun.simulationRunId)).orderBy(asc(simulationStageCheckpoints.semesterNumber), asc(simulationStageCheckpoints.stageOrder));
        expect(selectedCheckpoint).toBeTruthy();
        const facultyId = login.body.faculty.facultyId;
        const ownedOfferingRows = await current.db.select().from(facultyOfferingOwnerships).where(and(eq(facultyOfferingOwnerships.facultyId, facultyId), eq(facultyOfferingOwnerships.status, 'active')));
        ...
        const loadProfileForRole = async (roleCode, simulationStageCheckpointId) => {
        ...
        expect(courseLeaderCheckpointProfile.currentBatchContexts.every((item) => {
        ...
        const termById = new Map(allTermRows.map(row => [row.termId, row]));
        ...
        expect(courseLeaderCheckpointProfile.currentOwnedClasses.map((item) => item.offeringId).sort()).toEqual(checkpointOwnedOfferingIds);
        ...
        expect(courseLeaderProfile.currentOwnedClasses.map((item) => item.offeringId).sort()).toEqual(activeSemesterOwnedOfferingIds);
        expect(courseLeaderProfile.proofOperations.monitoringQueue.every((item) => activeSemesterOwnedOfferingIds.includes(item.offeringId))).toBe(true);
        if (courseLeaderCheckpointProfile.proofOperations.monitoringQueue[0]) {
            const queueItem = courseLeaderCheckpointProfile.proofOperations.monitoringQueue[0];
            const studentId = queueItem.studentId;
            const bootstrapStudent = (courseLeaderCheckpointBootstrap.studentsByOffering[queueItem.offeringId] ?? []).find((student) => {
        ...
        expect(mentorCheckpointProfile.currentBatchContexts.every((item) => {
        ...
        expect(mentorProfile.proofOperations.monitoringQueue.every((item) => mentorStudentIds.has(item.studentId))).toBe(true);
        expect(mentorProfile.proofOperations.electiveFits.every((item) => mentorStudentIds.has(item.studentId))).toBe(true);
        if (mentorProfile.proofOperations.monitoringQueue[0]) {
            const studentId = mentorProfile.proofOperations.monitoringQueue[0].studentId;
            const [riskExplorerResponse, studentShellResponse] = await Promise.all([
                current.app.inject({
                    method: 'GET',
                    url: `/api/academic/students/${studentId}/risk-explorer?simulationStageCheckpointId=${encodeURIComponent(selectedCheckpoint.simulationStageCheckpointId)}`,
    ...
    it('keeps academic playback checkpoints available when another proof run is also active', async () => {
        ...
        const [selectedCheckpoint] = await current.db.select().from(simulationStageCheckpoints).orderBy(asc(simulationStageCheckpoints.semesterNumber), asc(simulationStageCheckpoints.stageOrder));
        expect(selectedCheckpoint).toBeTruthy();
        const [checkpointRun] = await current.db.select().from(simulationRuns).where(eq(simulationRuns.simulationRunId, selectedCheckpoint.simulationRunId));
        ...
        const syntheticCheckpointSummary = {
            ...JSON.parse(selectedCheckpoint.summaryJson),
