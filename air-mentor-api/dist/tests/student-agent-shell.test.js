...
import { facultyAppointments, facultyOfferingOwnerships, mentorAssignments, roleGrants, simulationRuns, simulationStageCheckpoints, studentObservedSemesterStates, } from '../src/db/schema.js';
...
describe('student agent shell', () => {
    it('returns a deterministic card and bounded replies for in-scope course leaders', async () => {
        ...
        const observedRows = await current.db.select().from(studentObservedSemesterStates).where(and(eq(studentObservedSemesterStates.simulationRunId, activeRun.simulationRunId), eq(studentObservedSemesterStates.semesterNumber, 6)));
        ...
        const replyItemsOne = replyOne.json().items;
        const replyItemsTwo = replyTwo.json().items;
        const blockedItems = blockedReply.json().items;
        ...
        const checkpointItems = checkpointReply.json().items;
    ...
    it('enforces mentor and HoD scope and allows system admin archived-run inspection', async () => {
        ...
        const outsideHodGrantId = outsideHodLogin.body.availableRoleGrants.find((grant) => grant.roleCode === 'HOD')?.grantId;
        const outsideHodFacultyId = outsideHodRole.activeRoleGrant.facultyId;
    ...
    it('allows HOD student shell access when the active role grant remains in scope after appointment drift', async () => {
        ...
        const hodFacultyId = hodRole.activeRoleGrant.facultyId;
        ...
        const accessibleStudentId = hodStudentsResponse.json().items[0]?.studentId;
        expect(accessibleStudentId).toBeTruthy();
        await current.db.update(facultyAppointments).set({
            departmentId: 'dept_ece',
            branchId: 'branch_ece_btech',
    ...
    it('uses the activated proof semester for the default student shell while keeping checkpoint playback separate', async () => {
        ...
        const ownershipRows = await current.db.select().from(facultyOfferingOwnerships).where(and(eq(facultyOfferingOwnerships.facultyId, roleResponse.faculty.facultyId), eq(facultyOfferingOwnerships.status, 'active')));
        const ownedOfferingIds = new Set(ownershipRows.map(row => row.offeringId));
        const observedRows = await current.db.select().from(studentObservedSemesterStates).where(eq(studentObservedSemesterStates.simulationRunId, activeRun.simulationRunId));
        const accessibleStudentId = observedRows.find(row => {
            const offeringId = getObservedOfferingId(row);
            return !!offeringId && ownedOfferingIds.has(offeringId);
        })?.studentId;
