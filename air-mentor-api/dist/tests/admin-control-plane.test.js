...
import { academicTerms, academicRuntimeState, auditEvents, batches, branches, facultyAppointments, facultyCalendarAdminWorkspaces, facultyOfferingOwnerships, mentorAssignments, officialCodeCrosswalks, riskEvidenceSnapshots, roleGrants, sectionOfferings, simulationRuns, simulationQuestionTemplates, studentAssessmentScores, studentEnrollments, studentAttendanceSnapshots, studentBehaviorProfiles, studentCoStates, studentObservedSemesterStates, studentQuestionResults, studentTopicStates, studentInterventionResponseStates, transcriptSubjectResults, transcriptTermResults, } from '../src/db/schema.js';
...
async function switchRoleContext(cookie, roleGrantId) {
    if (!current)
...
function weekdayFromDateIso(value) {
    const parsed = new Date(value);
    const weekday = parsed.getUTCDay();
    return (['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][weekday] ?? null);
}
function timetableBlocksCanOverlap(left, right) {
...
function timetableRangesOverlap(left, right) {
    return left.startMinutes < right.endMinutes && right.startMinutes < left.endMinutes;
}
async function readPublishedOperationalProjectionCounts(simulationRunId) {
    if (!current)
        throw new Error('Test app is not initialized');
    const observedRows = await current.db.select().from(studentObservedSemesterStates).where(eq(studentObservedSemesterStates.simulationRunId, simulationRunId));
    const proofStudentIds = new Set(observedRows.map(row => row.studentId));
    const proofOfferingIds = new Set(observedRows
        .map(row => {
    ...
    const transcriptRows = (await current.db.select().from(transcriptTermResults)).filter(row => (proofStudentIds.has(row.studentId)
        && proofTermIds.has(row.termId)));
    const transcriptTermIds = new Set(transcriptRows.map(row => row.transcriptTermResultId));
    const transcriptSubjectCount = (await current.db.select().from(transcriptSubjectResults)).filter(row => (transcriptTermIds.has(row.transcriptTermResultId))).length;
    const attendanceCount = (await current.db.select().from(studentAttendanceSnapshots)).filter(row => (proofStudentIds.has(row.studentId)
        && proofOfferingIds.has(row.offeringId)
        && row.source.startsWith('proof-run:'))).length;
    const assessmentCount = (await current.db.select().from(studentAssessmentScores)).filter(row => (proofStudentIds.has(row.studentId)
        && proofOfferingIds.has(row.offeringId)
        && row.termId === 'term_mnc_sem6')).length;
    return {
...
describe('admin control plane routes', () => {
    ...
    it('keeps admin-created non-proof faculty out of teaching bootstrap while preserving admin master data', async () => {
        ...
        expect(appointmentCreate.statusCode).toBe(200);
        for (const roleCode of ['COURSE_LEADER', 'MENTOR']) {
        ...
        expect(publicFaculty.json().items.some((item) => item.facultyId === createdFaculty.facultyId)).toBe(false);
        ...
        const bootstrapFaculty = bootstrap.faculty.find((item) => item.facultyId === createdFaculty.facultyId);
        const teacherCard = bootstrap.teachers.find((item) => item.id === createdFaculty.facultyId);
        ...
        expect(profile.permissions.map((item) => item.roleCode).sort()).toEqual(['COURSE_LEADER', 'MENTOR']);
    ...
    it('previews and bulk-applies mentor assignment changes with confirmation, audit detail, and mentor-surface parity', async () => {
        ...
        if (!proofBatch)
            throw new Error('Expected proof batch to exist');
        ...
        if (!proofBranch)
            throw new Error('Expected proof branch to exist');
        const app = current.app;
        const createScopedMentorFaculty = async (username, displayName, employeeCode) => {
        ...
        const scopedStudents = studentsResponse.json().items.filter((student) => (student.activeAcademicContext?.batchId === MSRUAS_PROOF_BATCH_ID
        ...
        if (!reassignedStudent)
            throw new Error('Expected a scoped student for bulk mentor reassignment');
        ...
        expect(preview.studentIds.every((studentId) => activeTargetStudentIds.has(studentId))).toBe(true);
        ...
        const targetMentorGrantId = targetMentorLogin.body.availableRoleGrants.find((grant) => grant.roleCode === 'MENTOR')?.grantId;
        expect(targetMentorGrantId).toBeTruthy();
        if (!targetMentorGrantId)
            throw new Error('Expected the created target mentor to expose a mentor grant');
        ...
        const proofSemesterStudentIds = preview.studentIds.filter((studentId) => {
    ...
    proofRcIt('runs the proof control plane end to end and exposes proof operations on the faculty profile', async () => {
        ...
        const runId = createRunResponse.json().simulationRunId;
        ...
        expect(dashboard.imports.some((item) => (item.curriculumImportVersionId === createdImport.curriculumImportVersionId && item.status === 'approved'))).toBe(true);
        expect(dashboard.proofRuns.some((item) => (item.simulationRunId === runId && item.activeFlag))).toBe(true);
        ...
        const timetablePayload = timetableRuntimeRow ? JSON.parse(timetableRuntimeRow.payloadJson) : {};
        for (const [facultyId, template] of Object.entries(timetablePayload)) {
            ...
            for (let index = 0; index < classBlocks.length; index += 1) {
                ...
                for (let compareIndex = index + 1; compareIndex < classBlocks.length; compareIndex += 1) {
                    ...
                    if (!timetableBlocksCanOverlap(left, right))
                        continue;
                    expect(timetableRangesOverlap(left, right), `Proof timetable should not overlap for ${facultyId}: ${left.id} vs ${right.id}`).toBe(false);
                }
            }
        }
        ...
        if (!sem6CoState)
            throw new Error('Expected a semester-6 CO state row');
        const sem6CoPayload = JSON.parse(sem6CoState.stateJson);
        const relatedTemplates = questionTemplates.filter(template => {
            if (template.offeringId !== sem6CoState.offeringId)
                return false;
            const tags = JSON.parse(template.coTagsJson);
        ...
        const relatedResults = questionResults.filter(result => result.studentId === sem6CoState.studentId
        ...
        const sem6Observed = observedRows.find((row) => row.semesterNumber === 6);
        expect(sem6Observed).toBeTruthy();
        const sem6Payload = sem6Observed ? JSON.parse(sem6Observed.observedStateJson) : {};
        ...
        if (!studentId)
            throw new Error('Expected an observed student state for the proof run');
        ...
        const evidenceItems = evidenceResponse.json().items;
        ...
        expect(profileResponse.statusCode).toBe(200);
        const profile = profileResponse.json();
        expect(profile.proofOperations.activeRunContexts.some((item) => item.simulationRunId === runId)).toBe(true);
        ...
        const firstCheckpointId = (dashboard.activeRunDetail.checkpoints.find((item) => (item.openQueueCount ?? 0) > 0)
            ?? dashboard.activeRunDetail.checkpoints[0])?.simulationStageCheckpointId;
        ...
        const checkpointItems = checkpointListResponse.json().items;
        ...
        const checkpointOfferings = bootstrapCheckpointResponse.json().offerings;
        ...
        expect(facultyProfileCheckpointResponse.json().proofOperations.activeOperationalSemester).toBe(checkpointDetailOne.json().checkpoint.semesterNumber);
        expect(facultyProfileCheckpointResponse.json().currentBatchContexts.every((item) => (item.currentSemester === checkpointDetailOne.json().checkpoint.semesterNumber))).toBe(true);
    ...
    it('activates a proof operational semester without mutating checkpoint playback state', async () => {
        ...
        const activeRunId = dashboardBefore.json().activeRunDetail?.simulationRunId;
        ...
        expect(dashboardAfter.json().activeRunDetail.checkpoints.some((item) => item.semesterNumber === 6)).toBe(true);
        expect(dashboardAfter.json().lifecycleAudit.some((item) => (item.actionType === 'semester-activated'
            && item.payload?.activeOperationalSemester === 4))).toBe(true);
    }, 300000);
    ...
    it('prefers the most recently updated active proof run when duplicate active flags exist', async () => {
        ...
        expect(dashboardBefore.statusCode).toBe(200);
        const activeRunId = dashboardBefore.json().activeRunDetail?.simulationRunId;
        const activeRunSeed = dashboardBefore.json().activeRunDetail?.seed;
        const importVersionId = dashboardBefore.json().imports[0]?.curriculumImportVersionId;
        expect(activeRunId).toBeTruthy();
        ...
        const shadowRunId = shadowRunResponse.json().simulationRunId;
    ...
    it('supports reminders and audit search across admin-managed entities', async () => {
        ...
        expect(auditResponse.json().items.map((item) => item.action)).toEqual(['created', 'updated']);
    ...
    it('soft-deleting a faculty cascades appointments, permissions, ownerships, and mentor links out of the live teaching surface', async () => {
        ...
        if (!offering)
            throw new Error('Expected an unassigned CSE offering for the ownership test');
    ...
    it('previews and applies scoped mentor bulk assignment with confirmation and audit coverage', async () => {
        ...
        const mentorFaculty = facultyResponse.json().items.find((faculty) => (faculty.username === 'devika.shetty'
            && faculty.roleGrants.some(grant => grant.roleCode === 'MENTOR' && grant.status === 'active')));
        expect(mentorFaculty).toBeTruthy();
        if (!mentorFaculty)
            throw new Error('Expected seeded mentor faculty for bulk-apply test');
        ...
        const createdStudentIds = new Set(activeBulkAssignments
            .filter(row => row.effectiveFrom === '2026-03-20' && row.effectiveTo === null)
            .map(row => row.studentId));
        expect(previewBody.students.filter((student) => student.action !== 'keep').length).toBeGreaterThan(0);
        expect(previewBody.students
            .filter((student) => student.action !== 'keep')
            .every((student) => createdStudentIds.has(student.studentId))).toBe(true);
        ...
        const mentorGrantId = mentorLogin.body.availableRoleGrants.find((grant) => grant.roleCode === 'MENTOR')?.grantId;
        ...
        expect(profileResponse.statusCode).toBe(200);
        const monitoringQueue = profileResponse.json().proofOperations.monitoringQueue;
        const targetedStudentIds = new Set(previewBody.studentIds);
    ...
    it('enforces the faculty timetable direct-edit window while keeping markers editable and reflected in teaching profile status', async () => {
        ...
        const offering = offeringsResponse.json().items.find((item) => item.offId === 'c3-A') ?? offeringsResponse.json().items[0];
        expect(offering).toBeTruthy();
        const template = {
            facultyId: 't1',
        ...
        const currentPayload = JSON.parse(calendarRow.payloadJson);
    ...
    proofRcIt('serves HoD proof analytics from the active proof run with faculty-profile parity and deterministic filters', async () => {
        ...
        if (!firstStudent)
            throw new Error('Expected at least one HoD student watch row');
        ...
        expect(reassessmentItems.every((item) => (item.studentId === firstStudent.studentId
            && item.simulationRunId === dashboard.activeRunDetail.simulationRunId))).toBe(true);
    });
    proofRcIt('scopes faculty proof operations to the active teaching role so proof links stay reachable', async () => {
        ...
        const facultyId = login.body.faculty.facultyId;
        ...
        const courseLeaderStudentIds = new Set(observedRows.flatMap(row => {
        ...
        const loadProfileForRole = async (roleCode) => {
            const grantId = login.body.availableRoleGrants.find((grant) => grant.roleCode === roleCode)?.grantId;
            ...
            expect(response.statusCode).toBe(200);
            return response.json().proofOperations;
        };
    ...
    proofRcIt('returns an empty HoD proof surface outside the supervised proof branch', async () => {
        ...
        if (!outsideBranch)
            throw new Error('Expected a non-proof branch for the outside HoD proof-scope test');
