...
describe('academic proof routes', () => {
    ...
    function extractWhereColumnName(condition) {
        if (!condition || typeof condition !== 'object' || !('queryChunks' in condition))
            return null;
        const queryChunks = condition.queryChunks;
        if (!Array.isArray(queryChunks))
            return null;
        const match = queryChunks.find(chunk => !!chunk && typeof chunk === 'object' && 'name' in chunk && typeof chunk.name === 'string');
    ...
    it('passes checkpoint playback identity through student-shell proof scope checks', async () => {
        ...
        await registerAcademicProofRoutes(app, context, {
