...
describe('academic bootstrap routes', () => {
    ...
    it('validates a requested playback checkpoint against the checkpoint run instead of the first active run row', async () => {
        ...
        await registerAcademicBootstrapRoutes(app, context, {
        ...
        expect(resolveAcademicStageCheckpoint).toHaveBeenCalledWith(context, expect.objectContaining({
            facultyId: 'mnc_t1',
            activeRoleGrant: expect.objectContaining({ roleCode: 'COURSE_LEADER' }),
        }), checkpoint.simulationRunId, checkpoint.simulationStageCheckpointId);
