...
describe('backend telemetry', () => {
    ...
    it('can mirror backend telemetry payloads to an external sink dispatch', async () => {
        ...
        expect(dispatch).toHaveBeenCalledWith('https://ops.example/ingest', expect.stringContaining('"name":"startup.ready"'), 'relay-secret');
    });
    ...
    it('persists backend telemetry when the app configures local retention', async () => {
        ...
        try {
            ...
            for (let attempt = 0; attempt < 20; attempt += 1) {
                ...
                if (persisted.some(item => item.source === 'backend' && item.name === 'startup.ready'))
                    break;
