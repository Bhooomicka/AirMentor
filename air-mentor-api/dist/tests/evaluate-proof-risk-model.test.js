...
import { blendProbabilityRows, buildHybridBlendPlan, chooseHybridBlendAlpha, buildQueueBurdenStageSummaries, evaluationPaths, queueRollupSectionKey, queueRollupStudentKey, } from '../scripts/evaluate-proof-risk-model.js';
describe('evaluate proof risk model helpers', () => {
    ...
    it('builds stage-specific hard-route plan with current fallback on empty slices', () => {
        ...
        const plan = buildHybridBlendPlan('attendanceRisk', {
            current: currentRows,
        ...
        expect(blendProbabilityRows([
            ...
            { label: 1, prob: 0.79 },
            { label: 0, prob: 0.19 },
        ], plan.byStage['pre-tt1'].alpha)).toEqual([
            { label: 1, prob: 0.79 },
            { label: 0, prob: 0.19 },
        ]);
    });
    it('supports custom evaluation output paths for concurrent runs', () => {
        ...
        try {
            ...
            if (previousDir === undefined)
                delete process.env.AIRMENTOR_EVAL_OUTPUT_DIR;
            else
                process.env.AIRMENTOR_EVAL_OUTPUT_DIR = previousDir;
            if (previousStem === undefined)
                delete process.env.AIRMENTOR_EVAL_OUTPUT_STEM;
            else
                process.env.AIRMENTOR_EVAL_OUTPUT_STEM = previousStem;
        }
    });
});
