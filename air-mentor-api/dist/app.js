...
export async function buildApp(options) {
    ...
    const disposeTelemetryPersistence = configureOperationalTelemetryPersistence(event => persistOperationalTelemetryEvent(context.db, event, context.now()));
    ...
    app.addHook('onRequest', async (request, reply) => {
        ...
        if (requestPath === '/api/client-telemetry')
            return;
        ...
        else if (!secureTokenEquals(csrfCookie, expectedToken))
            failureReason = 'cookie_mismatch';
        else if (!secureTokenEquals(csrfHeader, expectedToken))
            failureReason = 'header_mismatch';
        if (!failureReason)
            return;
    ...
    modules.push(registerSessionRoutes, registerInstitutionRoutes, registerAdminStructureRoutes, registerPeopleRoutes, registerStudentRoutes, registerCourseRoutes, registerAdminRequestRoutes, registerAdminProofSandboxRoutes, registerClientTelemetryRoutes, registerAcademicRoutes, registerAdminControlPlaneRoutes);
...
export function sendCookie(reply, name, value, secure, sameSite, expiresAt, httpOnly = true) {
