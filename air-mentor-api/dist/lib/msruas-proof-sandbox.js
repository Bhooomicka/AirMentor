...
import { academicCalendarAuditEvents, academicRuntimeState, academicTaskPlacements, academicTaskTransitions, academicTasks, academicTerms, alertDecisions, alertOutcomes, batches, branches, bridgeModules, courseTopicPartitions, courses, curriculumCourses, curriculumEdges, curriculumImportVersions, curriculumNodes, departments, electiveBaskets, electiveOptions, electiveRecommendations, facultyAppointments, facultyCalendarWorkspaces, facultyOfferingOwnerships, facultyProfiles, institutions, mentorAssignments, reassessmentEvents, riskAssessments, roleGrants, sectionOfferings, simulationResetSnapshots, simulationRuns, semesterTransitionLogs, studentAcademicProfiles, studentAssessmentScores, studentAttendanceSnapshots, studentEnrollments, studentInterventions, studentLatentStates, studentObservedSemesterStates, students, teacherAllocations, teacherLoadProfiles, transcriptSubjectResults, transcriptTermResults, uiPreferences, userAccounts, userPasswordCredentials, } from '../db/schema.js';
import { inferObservableRisk } from './inference-engine.js';
import { buildMonitoringDecision } from './monitoring-engine.js';
import { buildCompletenessCertificate, buildCurriculumOutputChecksum, compileMsruasCurriculumWorkbook, EMBEDDED_CURRICULUM_SOURCE_PATH, validateCompiledCurriculum, } from './msruas-curriculum-compiler.js';
import { calculateCgpa, calculateSgpa, evaluateCourseStatus, } from './msruas-rules.js';
import { hashPassword } from './passwords.js';
const curriculumSeed = curriculumSeedJson;
...
export async function ensureMsruasProofBatchStructure(db, now) {
...
export async function ensureMsruasProofSandboxSeeded(db, options) {
    const [existingBatch] = await db.select().from(batches).where(eq(batches.batchId, MSRUAS_PROOF_BATCH_ID));
    if (existingBatch) {
        return {
            seeded: false,
    ...
    const resolvedInstitutionId = options.institutionId ?? (await db.select().from(institutions).limit(1))[0]?.institutionId;
...
function courseCodeForSeed(course) {
...
function semesterCourses(semesterNumber) {
...
function buildStudentTrajectory(index) {
...
function teacherEffect(facultyId, course, sectionCode) {
...
function simulateSemesterCourse(input) {
    ...
    const attendancePct = clamp(Math.round(58
        ...
        - (difficulty * 8)
        + stableBetween(`${student.studentId}-${course.internalCompilerId}-attendance`, -7, 9)), 52, 98);
    const cePct = clamp(24
        + (mastery * 60)
        + (student.latentBase.selfRegulation * 10)
        + (prereq * 8)
        - (difficulty * 9)
        + stableBetween(`${student.studentId}-${course.internalCompilerId}-ce`, -12, 10), 10, 97);
    const seePct = clamp(22
        + (mastery * 58)
        + (prereq * 10)
        - (difficulty * 10)
        + stableBetween(`${student.studentId}-${course.internalCompilerId}-see`, -14, 12), 8, 98);
    ...
    for (const [facultyId, entries] of loadsByFacultyId.entries()) {
        ...
        const sortedEntries = [...entries].sort((left, right) => {
            if (left.offeringId !== right.offeringId)
                return left.offeringId.localeCompare(right.offeringId);
...
export async function seedMsruasProofSandbox(db, options) {
    ...
    const courseRows = curriculumSeed.courses.map(course => ({
    ...
    const curriculumNodeRows = curriculumSeed.courses.map((course, index) => ({
    ...
    const bridgeModuleRows = curriculumSeed.courses
    ...
    if (bridgeModuleRows.length > 0)
        await db.insert(bridgeModules).values(bridgeModuleRows);
    const topicPartitionRows = curriculumSeed.courses.flatMap(course => {
    ...
    const sem6OfferingRows = [];
    const sem6OwnershipRows = [];
    const offeringFacultyById = new Map();
    const activeOfferingLoads = new Map();
    ...
    const studentRows = [];
    const academicProfileRows = [];
    const mentorAssignmentRows = [];
    const enrollmentRows = [];
    const transcriptTermRows = [];
    const transcriptSubjectRows = [];
    const latentStateRows = [];
    const observedStateRows = [];
    const transitionRows = [];
    const attendanceRows = [];
    const assessmentRows = [];
    const riskRows = [];
    const reassessmentRows = [];
    const alertDecisionRows = [];
    const alertOutcomeRows = [];
    const interventionRows = [];
    const electiveRecommendationRows = [];
    const taskRows = [];
    const taskTransitionRows = [];
    const taskPlacementRows = [];
    const teacherAllocationRows = [];
    const teacherLoadRows = [];
    const courseFacultyBySemester = new Map();
    ...
    trajectories.forEach((trajectory, studentIndex) => {
        ...
        sem6OfferingRows
            ...
            if (!faculty || !course)
                return;
            const attendancePct = clamp(Math.round(60 + (trajectory.latentBase.attendanceDiscipline * 28) + (trajectory.latentBase.selfRegulation * 7) - (offeringIndex % 3) * 2 + stableBetween(`${trajectory.studentId}-${offering.offeringId}-att`, -9, 7)), 58, 97);
            const tt1Pct = clamp(26 + (trajectory.latentBase.academicPotential * 42) + (trajectory.latentBase.computingFoundation * 12) + (trajectory.latentBase.mathematicsFoundation * 12) + stableBetween(`${trajectory.studentId}-${offering.offeringId}-tt1`, -18, 12), 18, 96);
    ...
    if (reassessmentRows.length > 0)
        await db.insert(reassessmentEvents).values(reassessmentRows);
    ...
    if (interventionRows.length > 0)
        await db.insert(studentInterventions).values(interventionRows);
    ...
    const mergedTimetable = currentTimetableSlice[0]
        ? { ...JSON.parse(currentTimetableSlice[0].payloadJson), ...timetablePayload }
        : timetablePayload;
    ...
    const mergedCalendar = currentCalendarSlice[0]
        ? { ...JSON.parse(currentCalendarSlice[0].payloadJson), ...markerPayload }
        : markerPayload;
