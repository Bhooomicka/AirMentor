import { academicTerms, alertAcknowledgements, alertDecisions, batches, branches, courses, departments, electiveRecommendations, facultyAppointments, facultyOfferingOwnerships, facultyProfiles, mentorAssignments, reassessmentEvents, reassessmentResolutions, riskAssessments, riskOverrides, roleGrants, sectionOfferings, simulationRuns, simulationStageCheckpoints, simulationStageQueueProjections, simulationStageStudentProjections, students, studentInterventions, studentObservedSemesterStates, teacherAllocations, teacherLoadProfiles, transcriptTermResults, } from '../db/schema.js';
...
export async function buildHodProofAnalytics(db, input, deps) {
    const { average, buildEvidenceTimelineFromRows, bucketBacklogCount, hoursBetween, isOpenReassessmentStatus, matchesTextFilter, normalizeFilterValue, parseProofCheckpointSummary, proofRecoveryStateFromResolutionRow, proofResolutionPayloadFromRow, queueProjectionAssignedFacultyId, roundToOne, uniqueSorted, withProofPlaybackGate, } = deps;
    const [allAppointmentRows, allRunRows, batchRows, branchRows, departmentRows, termRows, facultyRows, grantRows, ownershipRows, mentorRows, studentProfileRows, courseRows, sectionOfferingRows, observedRows, riskAssessmentRows, reassessmentRows, alertRows, acknowledgementRows, resolutionRows, overrideRows, loadRows, allocationRows, interventionRows, electiveRows, transcriptRows, stageCheckpointRows, stageStudentRows, stageQueueRows,] = await Promise.all([
    ...
    const scopeBranchIds = new Set(activeAppointments.map(row => row.branchId).filter((value) => !!value));
    grantRows
    ...
    const departmentScopeKey = (departmentId) => {
        if (!departmentId)
            return null;
    ...
    const scopeDepartmentKeys = new Set(Array.from(scopeDepartmentIds).map(departmentScopeKey).filter((value) => !!value));
    const scopeBranchKeys = new Set(Array.from(scopeBranchIds).map(branchScopeKey).filter((value) => !!value));
    const matchesScopedDepartment = (departmentId) => {
        if (!departmentId)
            return false;
        if (scopeDepartmentIds.has(departmentId))
            return true;
        const key = departmentScopeKey(departmentId);
        return !!key && scopeDepartmentKeys.has(key);
    };
    const matchesScopedBranch = (branchId) => {
        if (!branchId)
            return false;
        if (scopeBranchIds.has(branchId))
            return true;
        const key = branchScopeKey(branchId);
        return !!key && scopeBranchKeys.has(key);
    };
    const matchesScopedAppointment = (departmentId, branchId) => matchesScopedDepartment(departmentId) || matchesScopedBranch(branchId);
    ...
    const operationalCheckpointSummary = activeRunId
        ? resolveOperationalCheckpointSummary(stageCheckpointRows.filter(row => row.simulationRunId === activeRunId), currentSemester, { parseProofCheckpointSummary, withProofPlaybackGate })
        : null;
    const activeTermIds = new Set(termRows
    ...
    if (input.filters?.simulationStageCheckpointId) {
        if (!activeRun || !activeBatch || !activeBranch || !activeRunId || !scopeMatchesActiveBatch)
            return emptyResponse;
        const checkpoint = requestedCheckpoint;
        if (!checkpoint)
            return emptyResponse;
        const checkpointSummary = withProofPlaybackGate(stageCheckpointRows
            .filter(row => row.simulationRunId === activeRunId)
            .sort((left, right) => left.semesterNumber - right.semesterNumber || left.stageOrder - right.stageOrder)
            .map(parseProofCheckpointSummary)).find(item => item.simulationStageCheckpointId === checkpoint.simulationStageCheckpointId)
        ...
        const checkpointQueueRows = stageQueueRows
            ...
            .filter(row => !input.filters?.studentId || row.studentId === input.filters.studentId);
        const checkpointQueueGovernance = (row) => {
        ...
        const latestCheckpointTranscriptByStudent = new Map();
        transcriptRows
        ...
        const facultyPermissionMap = new Map();
        ...
        const facultyIdsInScope = uniqueSorted([
            ...
            if (facultyAppointments.length === 0)
                return false;
        ...
        const courseRollups = Array.from(new Map(checkpointStudentRows.map(row => {
            ...
            return [`${row.courseCode}::${row.sectionCode}`, {
                    ...
                    averageAttendancePct: roundToOne(average(checkpointStudentRows.filter(item => item.courseCode === row.courseCode).map(item => {
                        const p = parseJson(item.projectionJson, {});
                        const evidence = (p.currentEvidence ?? {});
                        return Number(evidence.attendancePct ?? 0);
                    }))),
                    tt1WeakCount: checkpointStudentRows.filter(item => {
                        if (item.courseCode !== row.courseCode)
                            return false;
                        const p = parseJson(item.projectionJson, {});
                        const evidence = (p.currentEvidence ?? {});
                    ...
                    weakQuestionSignalCount: checkpointStudentRows.filter(item => item.courseCode === row.courseCode && Number((parseJson(item.projectionJson, {}).currentEvidence ?? {}).weakQuestionCount ?? 0) >= 4).length,
                    backlogCarryoverCount: checkpointStudentRows.filter(item => item.courseCode === row.courseCode && Number((parseJson(item.projectionJson, {}).currentStatus ?? {}).backlogCount ?? 0) > 0).length,
        ...
        const studentWatchRows = Array.from(new Set(checkpointStudentRows.map(row => row.studentId)))
            ...
            const rowsForStudent = checkpointStudentRows
                ...
                if (leftCounts !== rightCounts)
                    return Number(rightCounts) - Number(leftCounts);
            ...
            if (!primary)
                return null;
            const primaryPayload = parseJson(primary.projectionJson, {});
            const primaryEvidence = (primaryPayload.currentEvidence ?? {});
            const currentStatus = (primaryPayload.currentStatus ?? {});
            const governance = (primaryPayload.governance ?? {});
            const counterfactualPolicy = (primaryPayload.counterfactualPolicyDiagnostics ?? {});
            ...
            return {
                ...
                counterfactualLiftScaled: Number(counterfactualPolicy.counterfactualLiftScaled
                ...
                courseSnapshots: rowsForStudent.map(row => {
                    ...
                    return {
                        ...
                        queueState: typeof (payload.currentStatus ?? {}).queueState === 'string'
                            ? String((payload.currentStatus ?? {}).queueState)
                            : null,
                        queueCaseId: typeof ((payload.governance ?? {}).queueCaseId) === 'string'
                            ? String(((payload.governance ?? {}).queueCaseId))
                            : null,
                        primaryCase: (payload.governance ?? {}).primaryCase === true,
                        countsTowardCapacity: (payload.governance ?? {}).countsTowardCapacity === true,
                        governanceReason: typeof ((payload.governance ?? {}).governanceReason) === 'string'
                            ? String(((payload.governance ?? {}).governanceReason))
                            : null,
                        supportingCourseCount: Number.isFinite(Number(((payload.governance ?? {}).supportingCourseCount)))
                            ? Number(((payload.governance ?? {}).supportingCourseCount))
                            : 0,
                        assignedFacultyId: typeof ((payload.governance ?? {}).assignedFacultyId) === 'string'
                            ? String(((payload.governance ?? {}).assignedFacultyId))
                            : null,
                        riskChangeFromPreviousCheckpointScaled: Number(payload.riskChangeFromPreviousCheckpointScaled ?? 0),
                        counterfactualLiftScaled: Number(payload.counterfactualLiftScaled
                            ?? ((payload.counterfactualPolicyDiagnostics ?? {}).counterfactualLiftScaled)
        ...
        return {
            summary: {
                ...
                electiveDistribution: checkpoint.stageKey === 'post-see'
                    ? Array.from(new Map(filterElectiveRecommendationsForSemester(electiveRows, {
                        simulationRunId: activeRunId,
                        semesterNumber: checkpoint.semesterNumber,
                    })
                        .map(row => [row.stream, {
    ...
    if (input.filters?.studentId)
        distinctStudentIds.add(input.filters.studentId);
    const distinctStudentSectionById = new Map();
    observedRows
        .filter(row => row.simulationRunId === activeRunId)
        .forEach(row => {
        if (!distinctStudentSectionById.has(row.studentId))
            distinctStudentSectionById.set(row.studentId, row.sectionCode);
    });
    ...
    const facultyRowsForHod = facultyIdsInScope
        ...
        const relevantReassessments = activeReassessments.filter(row => (row.assignedFacultyId != null && row.assignedFacultyId === facultyId) || relevantRiskIds.has(row.riskAssessmentId));
        ...
        .sort((left, right) => (right.queueLoad - left.queueLoad) || (right.weeklyContactHours - left.weeklyContactHours) || left.facultyName.localeCompare(right.facultyName));
    const courseRollups = Array.from(new Map(uniqueSorted(activeOfferings.map(row => row.courseId)).map(courseId => {
        ...
        const weakQuestionValues = courseObserved.map(row => Number(parseObservedStateRow(row).questionEvidenceSummary?.weakQuestionCount ?? 0)).filter(Number.isFinite);
        ...
        .sort((left, right) => ((right.riskCountHigh + right.riskCountMedium) - (left.riskCountHigh + left.riskCountMedium)) || left.courseCode.localeCompare(right.courseCode));
    const latestTranscriptByStudent = new Map();
    transcriptRows
        .filter(row => {
        ...
        .forEach(row => {
        if (!latestTranscriptByStudent.has(row.studentId))
            latestTranscriptByStudent.set(row.studentId, row);
    });
    ...
    const latestReassessmentByRiskId = new Map();
    activeReassessments
        .slice()
        .sort((left, right) => {
    ...
    const studentWatchRows = Array.from(new Set(activeRiskRows.map(row => row.studentId)))
        ...
        const primaryReassessmentPayload = primaryReassessment ? parseJson(primaryReassessment.payloadJson, {}) : {};
        ...
        const courseSnapshots = riskForStudent.map(row => {
            ...
            const rowPayload = rowReassessment ? parseJson(rowReassessment.payloadJson, {}) : {};
            return {
                ...
                observedEvidence: {
                    ...
                    weakQuestionCount: Number(rowEvidence.questionEvidenceSummary?.weakQuestionCount ?? 0),
                    ...
                    interventionRecoveryStatus: rowEvidence.interventionResponse && typeof rowEvidence.interventionResponse === 'object'
                        ? String(rowEvidence.interventionResponse.recoveryConfirmed ? 'confirmed' : 'watch')
                        : null,
                },
        ...
        return {
            ...
            observedEvidence: {
                ...
                weakCoCount: Number(primaryEvidence.weakCoCount ?? 0),
                weakQuestionCount: Number(primaryEvidence.questionEvidenceSummary?.weakQuestionCount ?? 0),
                coEvidenceMode: typeof primaryEvidence.coEvidenceMode === 'string' ? primaryEvidence.coEvidenceMode : null,
                interventionRecoveryStatus: primaryEvidence.interventionResponse && typeof primaryEvidence.interventionResponse === 'object'
                    ? String(primaryEvidence.interventionResponse.recoveryConfirmed ? 'confirmed' : 'watch')
                    : null,
            },
            electiveFit,
            courseSnapshots,
    ...
    const reassessments = activeReassessments
        ...
        if (!matchesTextFilter(risk.riskBand, input.filters?.riskBand))
            return null;
        if (!matchesTextFilter(student?.studentId ?? null, input.filters?.studentId))
            return null;
        if (!matchesTextFilter(course?.courseCode ?? null, input.filters?.courseCode))
            return null;
        if (!matchesTextFilter(offering?.sectionCode ?? null, input.filters?.section))
            return null;
        if (facultyFilterIds && facultyFilterIds.size > 0) {
            ...
            if (!relevantOfferingIds.has(risk.offeringId) && !relevantStudentIds.has(row.studentId))
                return null;
        }
    ...
    const electiveDistribution = Array.from(new Map(semesterScopedElectiveRows
        .map(row => [row.stream, {
            stream: row.stream,
