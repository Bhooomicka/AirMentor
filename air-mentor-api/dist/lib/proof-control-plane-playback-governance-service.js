import { buildActionPolicyComparison, buildDeterministicId, buildNoActionSnapshot, buildStageEvidenceSnapshot, ceShortfallLabel, dominantCoEvidenceMode, downstreamCarryoverLabelForSource, includedAttendanceForSourceStage, mapActionToTaskType, playbackCheckpointNowIso, prerequisiteSummaryForSource, seeShortfallLabel, } from './proof-control-plane-playback-service.js';
...
function buildStageCandidate(input, source, checkpoint, stage, sourceState) {
    ...
    const shouldSwitchAction = !sourceState.actionTaken
        || (policyComparison.recommendedAction != null && policyComparison.recommendedAction !== sourceState.actionTaken && ((evidence.interventionResponseScore ?? 0) < -0.03
            || stage.key === 'post-tt2'
    ...
    const assignment = facultyAssignmentForSource(source, monitoring.queueOwnerRole, input);
    return {
        checkpoint,
        stage,
        source,
...
export function buildPlaybackGovernanceArtifacts(input) {
    const studentProjectionRows = [];
    const queueProjectionRows = [];
    const queueCaseRows = [];
    const stageEvidenceRows = [];
    ...
    for (const semesterNumber of input.semesterNumbers) {
        ...
        for (const stage of input.stageDefs) {
            ...
            if (!checkpoint)
                continue;
            const stageCandidates = semesterSources.map(source => buildStageCandidate(input, source, checkpoint, stage, sourceStateByKey.get(sourceKeyForStageSource(source))));
            const governance = governProofQueueStage({
                ...
                candidates: stageCandidates.map(candidate => ({
                    ...
                    assignedRole: candidate.monitoring.queueOwnerRole,
                ...
                sectionStudentCountByKey: new Map([...input.sectionStudentCountBySemesterSection.entries()].filter(([key]) => key.startsWith(`${semesterNumber}::`))),
                facultyBudgetByKey: input.facultyBudgetByKey,
            });
            const effectiveDecisionByCaseKey = new Map();
            governance.decisions.forEach((decision, caseKey) => {
                ...
                if ((decision.status === 'open' || decision.status === 'opened')
                    && priorCaseState?.open
            ...
            effectiveDecisionByCaseKey.forEach((decision, caseKey) => {
                ...
                if (!primaryCandidate || decision.status === 'idle')
                    return;
                queueCaseRows.push({
