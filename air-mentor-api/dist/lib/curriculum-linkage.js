...
function buildDefaultCourseOutcomes(courseCode, courseTitle, topicPartitions) {
...
function buildManifestPrerequisites(manifest, course) {
...
export function getSupportedCurriculumManifest(manifestKey) {
    return supportedManifests[manifestKey];
}
export function buildManifestPayloadItems(manifestKey) {
...
function normalizePythonCandidate(candidate) {
    ...
    if (!curriculumCourseId || !targetCourseCode || !sourceCourseCode || !rationale)
        return null;
    return {
        curriculumCourseId,
        targetCourseCode,
        ...
        sources: Array.isArray(candidate.sources)
            ? candidate.sources.filter((value) => typeof value === 'string' && value.trim().length > 0)
            : [],
...
function normalizePythonCandidates(rawCandidates) {
...
function findManifestPayloadForCourse(manifestItems, course) {
    return manifestItems.find(item => item.courseCode.toLowerCase() === course.courseCode.toLowerCase()
        || normalizeText(item.title) === normalizeText(course.title)) ?? null;
}
function buildCourseTextBundle(course) {
    return [
        course.courseCode,
        course.title,
...
function semanticOverlapScore(source, target) {
...
async function queryOllamaForLinkageSuggestions(input) {
    ...
    const prompt = [
        ...
        '',
        'Candidate sources:',
        ...input.candidateSources.map(course => (`- ${course.courseCode} | ${course.title} | sem ${course.semesterNumber} | topics ${[...course.topicPartitions.tt1, ...course.topicPartitions.tt2, ...course.topicPartitions.see].join(' ; ')}`)),
    ].join('\n');
    ...
    for (const target of scopedTargets) {
        ...
        if (manifestMatch) {
            for (const prerequisite of manifestMatch.prerequisites) {
                if (activePrerequisiteCodes.has(prerequisite.sourceCourseCode.toLowerCase()))
                    continue;
        ...
        if (llmSources.length > 0 && scopedTargets.length <= 6) {
            try {
                ...
                for (const proposal of proposals) {
                    if (activePrerequisiteCodes.has(proposal.sourceCourseCode.toLowerCase()))
                        continue;
                    const sourceCourse = byCourseCode.get(proposal.sourceCourseCode.toLowerCase());
                    if (!sourceCourse || sourceCourse.semesterNumber >= target.semesterNumber)
                        continue;
