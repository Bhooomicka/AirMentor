...
import { z } from 'zod';
import { academicFaculties, academicTerms, batches, batchCurriculumFeatureBindings, batchCurriculumFeatureOverrides, bridgeModules, branches, courseOutcomeOverrides, courseTopicPartitions, courses, curriculumFeatureProfileCourses, curriculumFeatureProfiles, curriculumLinkageCandidates, curriculumEdges, curriculumImportVersions, curriculumNodes, curriculumCourses, departments, electiveBaskets, electiveOptions, facultyAppointments, facultyOfferingOwnerships, institutions, mentorAssignments, studentEnrollments, policyOverrides, reassessmentEvents, roleGrants, riskAssessments, sectionOfferings, simulationRuns, stagePolicyOverrides, } from '../db/schema.js';
...
import { buildCurriculumLinkageCandidates, buildManifestPayloadItems, supportedCurriculumManifestKeySchema, } from '../lib/curriculum-linkage.js';
import { enqueueProofSimulationRun } from '../lib/proof-run-queue.js';
import { canonicalizeStagePolicy, decodeSectionScopeId, DEFAULT_STAGE_POLICY, encodeSectionScopeId, normalizeSectionCode, scopeTypeValues, stagePolicyPayloadSchema, } from '../lib/stage-policy.js';
import { ensureMsruasProofBatchStructure, MSRUAS_PROOF_BATCH_ID, } from '../lib/msruas-proof-sandbox.js';
...
function mapAcademicFaculty(row) {
    return {
...
function mapBatch(row) {
    return {
        ...
        sectionLabels: parseJson(row.sectionLabelsJson, []),
        status: row.status,
        version: row.version,
        createdAt: row.createdAt,
        updatedAt: row.updatedAt,
...
function mapCurriculumCourse(row) {
    return {
...
function mapPolicyOverride(row) {
    return {
...
function mapStagePolicyOverride(row) {
    return {
...
function mapCurriculumFeatureProfile(row) {
    return {
...
function mapBatchCurriculumFeatureBinding(row) {
    if (!row)
        return null;
    return {
        batchId: row.batchId,
...
function mapCurriculumFeatureProfileCourse(row) {
    return {
...
function mapBatchCurriculumFeatureOverride(row) {
    return {
...
function mapCurriculumLinkageCandidate(row) {
    return {
        ...
        signalSummary: parseJson(row.signalSummaryJson, {}),
        status: row.status,
...
function normalizeFeatureStringList(items) {
    return Array.from(new Set(items.map(item => item.trim()).filter(Boolean)));
}
function normalizeCurriculumFeaturePayload(payload) {
    return {
...
function createEmptyProofRefresh(curriculumImportVersionId = null) {
    return {
        affectedBatchIds: [],
...
function formatCurriculumEdgeValidationMessage(errors) {
...
function findCurriculumNodeForCourse(nodeRows, course) {
    return nodeRows.find(row => (course.courseId && row.courseId === course.courseId)
        || (row.semesterNumber === course.semesterNumber && row.courseCode.toLowerCase() === course.courseCode.toLowerCase())
        || (row.semesterNumber === course.semesterNumber && row.title.toLowerCase() === course.title.toLowerCase())) ?? null;
}
async function ensureCourseRecordForCurriculumCourse(context, curriculumCourse) {
    ...
    else if (course.courseCode !== curriculumCourse.courseCode
...
async function ensureEditableCurriculumImport(context, input) {
...
async function upsertCurriculumNodeForCourse(context, input) {
...
async function refreshCurriculumImportSummary(context, input) {
...
async function syncCurriculumCourseIntoImport(context, input) {
    const latestImport = await getLatestCurriculumImport(context, input.curriculumCourse.batchId);
    if ((input.curriculumCourse.status === 'deleted' || input.curriculumCourse.status === 'archived') && !latestImport)
        return null;
...
function listBatchSectionLabels(batch) {
    return parseJson(batch.sectionLabelsJson, [])
        .map(normalizeSectionCode)
        .filter(Boolean);
}
function resolveBatchSectionScope(batch, requestedSectionCode) {
...
function scopeReferencesDeletedBatch(scopeId, batchIds) {
    const parsed = decodeSectionScopeId(scopeId);
    return parsed ? batchIds.has(parsed.batchId) : false;
}
async function getBatchScopeContext(context, batchId, requestedSectionCode) {
    ...
    return {
        ...
        scopeChain: [
            ...
            ...(sectionScope ? [{ scopeType: 'section', scopeId: sectionScope.scopeId }] : []),
        ],
    };
}
...
function describeScopeDescriptor(scopeContext, scope) {
    if (scope.scopeType === 'institution')
        return scopeContext.institution.name;
    if (scope.scopeType === 'academic-faculty')
        return scopeContext.department.academicFacultyId ?? scope.scopeId;
    if (scope.scopeType === 'department')
        return scopeContext.department.name;
    if (scope.scopeType === 'branch')
        return scopeContext.branch.name;
    if (scope.scopeType === 'batch')
        return scopeContext.batch.batchLabel;
    return `Section ${scopeContext.sectionCode ?? scope.scopeId.split('::').at(-1) ?? scope.scopeId}`;
}
function buildResolvedPolicyProvenance(scopeContext, appliedOverrides) {
    const activeScope = scopeContext.scopeChain.at(-1) ?? { scopeType: 'batch', scopeId: scopeContext.batch.batchId };
...
function matchesCourseReference(input, candidate) {
    return ((!!input.courseId && !!candidate.courseId && input.courseId === candidate.courseId)
        || input.courseCode.toLowerCase() === candidate.courseCode.toLowerCase()
        || input.title.toLowerCase() === candidate.title.toLowerCase());
}
async function loadMaterializedCurriculumFeatureBundle(context, batchId) {
    const [batch] = await context.db.select().from(batches).where(eq(batches.batchId, batchId));
    if (!batch)
        throw notFound('Batch not found');
    ...
    const items = [...visibleCurriculumRows, ...nodeOnlyRows].map(curriculumCourse => {
        ...
        const bridgeRow = node
            ? bridgeRows
                ...
                .sort((left, right) => right.updatedAt.localeCompare(left.updatedAt))[0] ?? null
            : null;
        const topicByKind = new Map((node ? topicRows.filter(row => row.curriculumNodeId === node.curriculumNodeId) : []).map(row => [
            row.partitionKind,
            normalizeFeatureStringList(parseJson(row.topicsJson, [])),
        ]));
        return {
            ...
            bridgeModules: bridgeRow ? normalizeFeatureStringList(parseJson(bridgeRow.moduleTitlesJson, [])) : [],
            topicPartitions: {
...
function toCurriculumFeaturePayload(item) {
...
function fromResolvedCurriculumFeaturePayload(payload, item, batchItems) {
    return {
...
function batchFeatureFingerprint(items) {
...
export async function resolveBatchStagePolicy(context, batchId, options = {}) {
    ...
    let effectivePolicy = DEFAULT_STAGE_POLICY;
...
export async function resolveBatchCurriculumFeatures(context, batchId) {
    ...
    const binding = mapBatchCurriculumFeatureBinding(bindingRowRaw) ?? {
        ...
        bindingMode: 'inherit-scope-profile',
        status: 'active',
        version: 1,
        createdAt: '',
        updatedAt: '',
    ...
    const items = materializedBundle.items.map(item => {
        ...
        if (binding.bindingMode !== 'local-only') {
            for (const profile of availableProfiles) {
                const profileCourse = profileCourses.find(row => (row.curriculumFeatureProfileId === profile.curriculumFeatureProfileId
        ...
        if (binding.bindingMode === 'pin-profile' && binding.curriculumFeatureProfileId) {
            const pinnedProfile = availableProfiles.find(profile => profile.curriculumFeatureProfileId === binding.curriculumFeatureProfileId)
                ...
                ?? null;
            const pinnedCourse = profileCourses.find(row => (row.curriculumFeatureProfileId === binding.curriculumFeatureProfileId
                && matchesCourseReference({
                    courseId: item.courseId,
                    courseCode: item.courseCode,
                    title: item.title,
...
async function materializeResolvedCurriculumFeatureItems(context, input) {
    ...
    for (const item of input.items) {
        ...
        for (const prerequisite of item.resolvedConfig.prerequisites) {
            ...
            if (sourceNode.curriculumNodeId === node.curriculumNodeId)
                continue;
...
function validateResolvedCurriculumFeatureItems(input) {
    ...
    for (const item of input.items) {
        ...
        for (const prerequisite of item.resolvedConfig.prerequisites) {
            ...
            if (sourceRow.curriculumCourseId === targetRow.curriculumCourseId
...
async function rematerializeResolvedBatchCurriculum(context, input) {
...
function expandCurriculumLinkageNeighborhood(input) {
    ...
    for (const targetId of input.targetCurriculumCourseIds) {
        ...
        targetItem.prerequisites.forEach(prerequisite => {
            ...
            if (sourceItem)
                expandedIds.add(sourceItem.curriculumCourseId);
        });
...
export async function enqueueProofRefreshForBatches(context, input) {
...
async function listBatchesInScope(context, scopeType, scopeId) {
    ...
    return allBatches.filter(batch => {
        ...
        if (scopeType === 'department')
            return department.departmentId === scopeId;
        if (scopeType === 'branch')
            return branch.branchId === scopeId;
        return batch.batchId === scopeId;
    });
}
function createAcademicFacultyCascadeSummary() {
    return {
...
async function assertScopeExists(context, scopeType, scopeId) {
    if (scopeType === 'institution') {
    ...
    if (scopeType === 'section') {
        const parsed = decodeSectionScopeId(scopeId);
        if (!parsed)
            throw badRequest('Section scope ids must use the canonical <batchId>::<SECTION> format.');
        const [batch] = await context.db.select().from(batches).where(eq(batches.batchId, parsed.batchId));
        if (!batch)
            throw notFound('Section scope not found');
        const knownSectionLabels = listBatchSectionLabels(batch);
        if (!knownSectionLabels.includes(parsed.sectionCode))
            throw notFound('Section scope not found');
        return {
            batch,
...
async function upsertCurriculumFeatureProfileCourseRecord(context, input) {
    const existingRows = await context.db.select().from(curriculumFeatureProfileCourses);
    const existing = existingRows.find(row => (row.curriculumFeatureProfileId === input.curriculumFeatureProfileId
        && matchesCourseReference({
            courseId: input.courseId,
            courseCode: input.curriculumCourse.courseCode,
            title: input.curriculumCourse.title,
...
async function regenerateCurriculumLinkageCandidatesForBatch(context, input) {
    const resolved = await resolveBatchCurriculumFeatures(context, input.batchId);
    ...
    const rowsToSupersede = existingRows.filter(row => (row.status === 'pending'
...
async function bootstrapCurriculumManifestForBatch(context, input) {
    ...
    const existingBySemesterAndCode = new Map(existingCurriculumRows.map(row => [`${row.semesterNumber}::${row.courseCode.toLowerCase()}`, row]));
    let createdCourseCount = 0;
    let upsertedProfileCourseCount = 0;
    for (const manifestCourse of manifestItems) {
        const key = `${manifestCourse.semesterNumber}::${manifestCourse.courseCode.toLowerCase()}`;
...
async function approveCurriculumLinkageCandidate(context, input) {
    ...
    if (!candidateRow || candidateRow.batchId !== input.batchId)
        throw notFound('Curriculum linkage candidate not found');
    if (candidateRow.status !== 'pending')
        throw badRequest('Only pending linkage candidates can be approved');
    const resolved = await resolveBatchCurriculumFeatures(context, input.batchId);
    const targetItem = resolved.items.find(item => item.curriculumCourseId === candidateRow.curriculumCourseId);
    if (!targetItem)
        throw notFound('Resolved curriculum feature item not found');
    ...
    const nextPayload = normalizeCurriculumFeaturePayload({
        ...
        prerequisites: [
            ...targetItem.resolvedConfig.prerequisites,
            {
                ...
                edgeKind: candidateRow.edgeKind,
    ...
    const rowsToSupersede = sameTargetPending.filter(row => (row.status === 'pending'
...
async function buildProofSandboxSummary(context, batchId) {
    const [importRows, nodeRows, edgeRows, bridgeRows, simulationRows, assessmentRows, reassessmentRows,] = await Promise.all([
    ...
    const latestRunMetrics = latestRun ? parseJson(latestRun.metricsJson, {}) : {};
    ...
    return {
        ...
        curriculumImport: latestImport
            ? {
                ...
                semesterRange: [latestImport.firstSemester, latestImport.lastSemester],
        ...
        latestSimulationRun: latestRun
            ? {
                ...
                semesterRange: [latestRun.semesterStart, latestRun.semesterEnd],
...
export async function resolveBatchPolicy(context, batchId, options = {}) {
    const scopeContext = await getBatchScopeContext(context, batchId, options.sectionCode ?? null);
...
export async function registerAdminStructureRoutes(app, context) {
    ...
    app.patch('/api/admin/academic-faculties/:academicFacultyId', {
        ...
        if (!current)
            throw notFound('Academic faculty not found');
        ...
        if (body.status === 'deleted' && current.status !== 'deleted') {
            const [departmentRows, branchRows, batchRows, termRows, courseRows, curriculumRows, policyRows, offeringRows, ownershipRows, appointmentRows, roleGrantRows,] = await Promise.all([
                context.db.select().from(departments),
                context.db.select().from(branches),
                context.db.select().from(batches),
            ...
            for (const row of policyRows.filter(item => ((item.scopeType === 'academic-faculty' && item.scopeId === params.academicFacultyId)
                ...
                || (item.scopeType === 'section' && scopeReferencesDeletedBatch(item.scopeId, batchIds))) && item.status !== 'deleted')) {
                const next = {
                    ...row,
                    status: 'deleted',
                    version: row.version + 1,
            ...
            for (const row of appointmentRows.filter(item => (departmentIds.has(item.departmentId)
                || (item.branchId ? branchIds.has(item.branchId) : false)) && item.status !== 'deleted')) {
                const next = {
                    ...row,
                    status: 'deleted',
                    version: row.version + 1,
            ...
            for (const row of roleGrantRows.filter(item => ((item.scopeType === 'academic-faculty' && item.scopeId === params.academicFacultyId)
                || (item.scopeType === 'department' && departmentIds.has(item.scopeId))
                || (item.scopeType === 'branch' && branchIds.has(item.scopeId))
                || (item.scopeType === 'batch' && batchIds.has(item.scopeId))
                || (item.scopeType === 'section' && scopeReferencesDeletedBatch(item.scopeId, batchIds))) && item.status !== 'deleted')) {
    ...
    app.get('/api/admin/batches/:batchId/setup-readiness', {
        ...
        const activeMentorGrants = allMentorGrants.filter(g => isVisibleStatus(g.status)
        ...
        const allOwnerships = offeringIds.length > 0
            ...
            : [];
        const ownedOfferingIds = new Set(allOwnerships.filter(ow => ow.status === 'active').map(ow => ow.offeringId));
        ...
        const studentsWithActiveEnrollment = new Set(scopedEnrollments.filter(e => e.endDate === null).map(e => e.studentId));
        ...
        const allMentorAssignments = batchStudentIdsArray.length > 0
            ...
            : [];
        const activeAssignmentStudentIds = new Set(allMentorAssignments.filter(a => a.effectiveTo === null).map(a => a.studentId));
        const studentsWithoutMentorCount = batchStudentIdsArray
            .filter(id => !activeAssignmentStudentIds.has(id)).length;
        const rosterKeySet = new Set(scopedEnrollments.filter(e => e.endDate === null).map(e => `${e.termId}::${e.sectionCode}`));
        ...
        if (!currentSemesterTerm)
            blockers.push(`Add the live semester term for ${batchLabel}.`);
        if (activeCurriculumRows.length === 0)
            blockers.push(`Add curriculum rows before moving ${batchLabel} forward.`);
        if (activeBranchAppointments.length === 0)
            blockers.push(`Add faculty appointments in scope for ${batchLabel}.`);
        if (uniqueMentorFacultyCount === 0)
            blockers.push(`Grant at least one mentor-ready faculty member in ${batchLabel}.`);
        if (batchStudentIds.size === 0)
            blockers.push(`Add or provision students for ${batchLabel}.`);
        if (visibleOfferings.length === 0)
            blockers.push(`Create teaching offerings for ${batchLabel}.`);
        if (offeringsWithoutOwnerCount > 0)
            blockers.push(`${offeringsWithoutOwnerCount} offering(s) still need a faculty owner.`);
        if (studentsWithoutEnrollmentCount > 0)
            blockers.push(`${studentsWithoutEnrollmentCount} student(s) still need an active enrollment.`);
        if (studentsWithoutMentorCount > 0)
            blockers.push(`${studentsWithoutMentorCount} student(s) still need a mentor.`);
        if (offeringsWithoutRosterCount > 0)
            blockers.push(`${offeringsWithoutRosterCount} offering(s) still have no roster.`);
    ...
    app.patch('/api/admin/curriculum-courses/:curriculumCourseId', {
        ...
        const now = context.now();
        const beforeResolvedByBatchId = new Map([
        ...
        if (!next)
            throw notFound('Curriculum course not found after update');
        await syncCurriculumCourseIntoImport(context, {
        ...
        let previousBatchImportVersionId = null;
    ...
    app.post('/api/admin/batches/:batchId/curriculum/linkage-candidates/:curriculumLinkageCandidateId/reject', {
        ...
        if (!current || current.batchId !== params.batchId)
            throw notFound('Curriculum linkage candidate not found');
        const now = context.now();
        await context.db.update(curriculumLinkageCandidates).set({
            status: 'rejected',
    ...
    app.put('/api/admin/batches/:batchId/curriculum-feature-config/:curriculumCourseId', {
        ...
        if (!curriculumCourse || curriculumCourse.batchId !== params.batchId)
            throw notFound('Curriculum course not found');
        const normalizedPayload = normalizeCurriculumFeaturePayload(body);
        const beforeBatchFingerprints = new Map();
        const now = context.now();
        const courseRecord = await ensureCourseRecordForCurriculumCourse(context, curriculumCourse);
        ...
        if (body.targetMode === 'scope-profile') {
            ...
            if (!targetScopeId)
                throw badRequest('Scope profile save requires a target scope id');
            ...
            for (const candidate of affectedCandidateBatches) {
                ...
                if (binding?.bindingMode === 'local-only' || binding?.status === 'archived')
                    continue;
                const localOverride = await context.db.select().from(batchCurriculumFeatureOverrides).where(eq(batchCurriculumFeatureOverrides.batchId, candidate.batchId)).then(rows => rows.find(row => (row.curriculumCourseId === params.curriculumCourseId
                    || row.courseId === courseRecord.courseId
                    || row.courseCode.toLowerCase() === curriculumCourse.courseCode.toLowerCase())) ?? null);
                if (localOverride?.status === 'active')
                    continue;
        ...
        if (!matchingItem)
            throw notFound('Resolved curriculum feature item not found after save');
    ...
    app.patch('/api/admin/curriculum-feature-profiles/:curriculumFeatureProfileId', {
        ...
        if (!current)
            throw notFound('Curriculum feature profile not found');
    ...
    app.put('/api/admin/batches/:batchId/curriculum-feature-binding', {
        ...
        if (body.curriculumFeatureProfileId) {
            ...
            if (!profile || profile.status !== 'active')
                throw notFound('Pinned curriculum feature profile not found');
        }
        const beforeResolved = await resolveBatchCurriculumFeatures(context, params.batchId);
        ...
        if (current) {
            if (body.version != null)
                expectVersion(current.version, body.version, 'BatchCurriculumFeatureBinding', mapBatchCurriculumFeatureBinding(current));
    ...
    app.patch('/api/admin/stage-policy-overrides/:stagePolicyOverrideId', {
        ...
        if (!current)
            throw notFound('Stage policy override not found');
    ...
    app.patch('/api/admin/policy-overrides/:policyOverrideId', {
        ...
        if (!current)
            throw notFound('Policy override not found');
