...
import { z } from 'zod';
import { academicTerms, batches, branches, departments, facultyAppointments, facultyProfiles, institutions, mentorAssignments, roleGrants, studentAcademicProfiles, studentEnrollments, students, } from '../db/schema.js';
    ...
    return {
        ...
        activeMentorAssignment: (() => {
            const activeAssignment = params.assignmentRows
                .filter(item => item.studentId === params.student.studentId)
                .sort((left, right) => {
                if (left.effectiveTo === null && right.effectiveTo !== null)
                    return -1;
                if (left.effectiveTo !== null && right.effectiveTo === null)
                    return 1;
...
function buildStudentScopeCacheKey(batchId, sectionCode) {
    return `${batchId}::${(sectionCode ?? '').trim().toUpperCase()}`;
}
async function enrichStudentRecordWithProvenance(context, student, cache) {
...
function matchesStudentDirectoryScope(student, academicFacultyByDepartmentId, filter) {
    ...
    if (filter.departmentId && context.departmentId !== filter.departmentId)
        return false;
    if (filter.branchId && context.branchId !== filter.branchId)
        return false;
    if (filter.batchId && context.batchId !== filter.batchId)
        return false;
    if (filter.sectionCode && normalizeSectionCode(context.sectionCode) !== normalizeSectionCode(filter.sectionCode))
        return false;
    return true;
}
function buildBulkMentorScopeLabel(batchLabel, sectionCode) {
    return sectionCode ? `Batch ${batchLabel} · Section ${sectionCode}` : `Batch ${batchLabel}`;
}
export async function registerStudentRoutes(app, context) {
    app.get('/api/admin/students', {
        ...
        const academicFacultyByDepartmentId = new Map(departmentRows.map(row => [row.departmentId, row.academicFacultyId ?? null]));
        ...
        return {
            items: await Promise.all(mappedStudents.map(student => enrichStudentRecordWithProvenance(context, student, provenanceCache))),
        };
    });
    ...
    app.post('/api/admin/mentor-assignments/bulk-apply', {
        ...
        const knownSectionLabels = parseJson(batch.sectionLabelsJson, [])
        ...
        if (!facultyEligibility.eligible) {
            throw badRequest(`Faculty ${selectedFaculty.displayName} is not mentor-eligible for ${buildBulkMentorScopeLabel(batch.batchLabel, sectionCode)}. ${facultyEligibility.reasons.join(' ')}`.trim(), { reasons: facultyEligibility.reasons });
        }
        ...
        const currentMentorAssignmentByStudentId = new Map();
        assignmentRows
        ...
        const scopedStudents = studentRecords
            ...
            .sort((left, right) => ((left.activeAcademicContext?.sectionCode ?? '').localeCompare(right.activeAcademicContext?.sectionCode ?? '')
            || (left.rollNumber ?? '').localeCompare(right.rollNumber ?? '')
            || left.usn.localeCompare(right.usn)
            || left.name.localeCompare(right.name)));
        const previewStudents = scopedStudents.flatMap(student => {
    ...
    app.patch('/api/admin/mentor-assignments/:assignmentId', {
        ...
        if (!current)
            throw notFound('Mentor assignment not found');
