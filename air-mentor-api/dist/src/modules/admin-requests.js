...
import { canAccessAdminRequest, createAdminRequestTransition, deserializeTargetEntityRefs, emitAuditEvent, expectVersion, findSystemAdminFacultyId, getAdminRequestNotes, getAdminRequestTransitions, getAuditEventsForEntity, parseOrThrow, prioritySchema, requireAuth, requireRole, serializeTargetEntityRefs, } from './support.js';
...
async function getRequestOrThrow(context, adminRequestId) {
    const [row] = await context.db.select().from(adminRequests).where(eq(adminRequests.adminRequestId, adminRequestId));
    if (!row)
        throw notFound('Admin request not found');
    return row;
}
async function insertNote(context, params) {
    const note = {
...
async function transitionRequest(context, params) {
...
export async function registerAdminRequestRoutes(app, context) {
    app.get('/api/admin/requests', {
        schema: { tags: ['admin-requests'], summary: 'List admin requests' },
    }, async (request) => {
        const auth = requireAuth(request);
        if (!['SYSTEM_ADMIN', 'HOD'].includes(auth.activeRoleGrant.roleCode))
            throw forbidden();
    ...
    app.post('/api/admin/requests', {
        ...
        if (!auth.facultyId)
            throw forbidden('Faculty context is required to create an admin request');
