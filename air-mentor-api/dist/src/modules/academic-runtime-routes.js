...
import { z } from 'zod';
import { academicCalendarAuditEvents, academicMeetings, academicRuntimeState, academicTaskPlacements, academicTaskTransitions, academicTasks, courseOutcomeOverrides, facultyCalendarWorkspaces, offeringAssessmentSchemes, offeringQuestionPapers, sectionOfferings, studentAssessmentScores, studentAttendanceSnapshots, } from '../db/schema.js';
...
export async function registerAcademicRuntimeRoutes(app, context, deps) {
    const { academicMeetingCreateSchema, academicMeetingParamsSchema, academicMeetingPatchSchema, academicRoleCodes, assessmentCommitParamsSchema, assessmentCommitSchema, assertCourseLeaderCanManageOffering, assertStudentEnrolledInOffering, assertViewerCanManageTask, assertViewerCanSuperviseStudent, attendanceCommitSchema, buildAcademicMeetingResponse, buildDefaultQuestionPaper, buildDefaultSchemeFromPolicy, calendarAuditSyncSchema, canonicalizeSchemeState, facultyCalendarWorkspaceUpsertSchema, flattenTermTestLeaves, getAcademicRuntimeState, getEditableCalendarWindowStatus, getOfferingContext, mapAcademicTaskRow, mapCalendarAuditEventRow, mapFacultyCalendarWorkspaceRow, mapTaskPlacementRow, mapTaskTransitionRow, millisToIso, normalizeAcademicStudentId, offeringParamsSchema, offeringQuestionPaperUpsertSchema, offeringSchemeUpsertSchema, questionPaperParamsSchema, resolveCourseOutcomesForOffering, runtimeSliceSchemas, runtimeStateKeySchema, saveAcademicRuntimeState, schemeStateSchema, taskPlacementSyncSchema, taskSyncSchema, termTestBlueprintSchema, validateFacultyCalendarTemplate, validateMeetingWindow, validateQuestionPaperBlueprint, validateSchemeAgainstPolicy, } = deps;
    ...
    const compatibilityRouteSuccessors = {
    ...
    function markCompatibilityRouteUsage(reply, route) {
    ...
    function taskRecordWithVersion(row, transitionRows) {
        return {
            ...mapAcademicTaskRow(row, transitionRows.map(mapTaskTransitionRow)),
            version: row.version,
        };
    }
    async function syncRuntimeTaskShadow(record, options = {}) {
        if (options.writeRuntimeShadow === false)
            return record;
        const { version: _version, ...runtimeComparableTask } = record;
        const currentTasks = await getAcademicRuntimeState(context, 'tasks');
        ...
        const savedTasks = await saveAcademicRuntimeState(context, 'tasks', nextTasks);
        const currentResolvedTasks = await getAcademicRuntimeState(context, 'resolvedTasks');
        ...
        return (savedTasks.find(task => task.id === runtimeComparableTask.id) ?? runtimeComparableTask);
    }
    async function syncRuntimeTaskPlacementShadow(taskId, placement, options = {}) {
        if (options.writeRuntimeShadow === false)
            return placement;
        const currentPlacements = await getAcademicRuntimeState(context, 'taskPlacements');
        ...
        const savedPlacements = await saveAcademicRuntimeState(context, 'taskPlacements', nextPlacements);
        return (savedPlacements[taskId] ?? null);
    }
    async function syncRuntimeCalendarAuditShadow(event, options = {}) {
        if (!event || options.writeRuntimeShadow === false)
            return event;
        const currentEvents = await getAcademicRuntimeState(context, 'calendarAudit');
        ...
        const savedEvents = await saveAcademicRuntimeState(context, 'calendarAudit', nextEvents);
        return (savedEvents.find(item => item.id === event.id) ?? event);
    }
    async function maybeEmitRuntimeShadowDrift(stateKey, entityId, authoritativeEntity) {
        const runtime = await getAcademicRuntimeState(context, stateKey);
        const runtimeEntity = stateKey === 'tasks'
            ? (runtime.find(item => item.id === entityId) ?? null)
            : stateKey === 'taskPlacements'
                ? ((runtime[entityId]) ?? null)
                : ((runtime.find(item => item.id === entityId)) ?? null);
        if (JSON.stringify(runtimeEntity) === JSON.stringify(authoritativeEntity))
            return;
    ...
    async function listVisibleTaskRecords(auth) {
        ...
        const transitionsByTaskId = new Map();
    ...
    async function persistAcademicTask(auth, taskInput, options = {}) {
        if (!auth.facultyId)
            throw forbidden('Faculty context is required');
        ...
        if (!storedTask)
            throw notFound('Task not found after save');
    ...
    async function persistAcademicTaskPlacement(auth, placementInput, options = {}) {
        if (!auth.facultyId)
            throw forbidden('Faculty context is required');
        const parsed = parseOrThrow(taskPlacementSyncSchema, {
            placements: {
                [String(placementInput?.taskId ?? '')]: placementInput,
            },
        });
        ...
        const [storedPlacement] = await context.db
            ...
            .where(eq(academicTaskPlacements.taskId, placement.taskId));
        if (!storedPlacement)
            throw notFound('Task placement not found after save');
    ...
    async function deleteAcademicTaskPlacement(auth, taskId, expectedUpdatedAt) {
        if (!auth.facultyId)
            throw forbidden('Faculty context is required');
        ...
        if (!current) {
            return { ok: true, taskId, deleted: false };
        }
    ...
    async function appendAcademicCalendarAuditEvent(auth, eventInput, options = {}) {
        if (!auth.facultyId)
            throw forbidden('Faculty context is required');
    ...
    app.put('/api/academic/runtime/:stateKey', {
        ...
        const body = parseOrThrow(runtimeSliceSchemas[params.stateKey], request.body);
    ...
    function registerNamedRuntimeSliceUpsertRoute(input) {
        app.put(input.route, {
            ...
            requireRole(request, [...academicRoleCodes]);
            const auth = requireAuth(request);
            const body = parseOrThrow(runtimeSliceSchemas[input.stateKey], request.body);
    ...
    app.put('/api/academic/faculty-calendar-workspace/:facultyId', {
        ...
        const timetablePayload = await getAcademicRuntimeState(context, 'timetableByFacultyId');
    ...
    app.post('/api/academic/meetings', {
        ...
        if (!saved)
            throw notFound('Meeting could not be created');
    ...
    app.patch('/api/academic/meetings/:meetingId', {
        ...
        const [saved] = await context.db
            ...
            .where(eq(academicMeetings.meetingId, params.meetingId));
        if (!saved)
            throw notFound('Meeting not found after update');
    ...
    app.put('/api/academic/offerings/:offeringId/attendance', {
        ...
        if (body.lock) {
            const currentLockPayload = await getAcademicRuntimeState(context, 'lockByOffering');
    ...
    app.put('/api/academic/offerings/:offeringId/assessment-entries/:kind', {
        ...
        if (params.kind === 'tt1' || params.kind === 'tt2') {
            ...
            const [paperRow] = await context.db
                ...
                .where(and(eq(offeringQuestionPapers.offeringId, params.offeringId), eq(offeringQuestionPapers.kind, params.kind)));
        ...
        for (const entry of body.entries) {
            const enrollment = await assertStudentEnrolledInOffering(context, offering, entry.studentId);
            await context.db.delete(studentAssessmentScores).where(and(eq(studentAssessmentScores.studentId, enrollment.studentId), eq(studentAssessmentScores.offeringId, params.offeringId), inArray(studentAssessmentScores.componentType, replacementComponentTypes)));
    ...
    app.post('/api/academic/offerings/:offeringId/assessment-entries/:kind/clear-lock', {
                ...
                : params.kind === 'quiz'
                    ...
                    : params.kind === 'assignment'
                        ...
                        : params.kind === 'finals'
                            ...
                            : null;
        if (!lockField)
            throw badRequest(`Invalid kind: ${params.kind}`);
        if (offering[lockField] !== 1) {
            return { ok: true, offeringId: params.offeringId, kind: params.kind, cleared: false, reason: 'already-unlocked' };
        }
        await context.db.update(sectionOfferings).set({
            [lockField]: 0,
            version: offering.version + 1,
        ...
        return { ok: true, offeringId: params.offeringId, kind: params.kind, cleared: true };
    });
