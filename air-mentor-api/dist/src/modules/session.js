...
async function buildSessionPayload(context, sessionId) {
...
function syncSessionCookies(context, reply, sessionId, expiresAt) {
    sendCookie(reply, context.config.sessionCookieName, sessionId, context.config.sessionCookieSecure, context.config.sessionCookieSameSite, expiresAt);
    sendCookie(reply, context.config.csrfCookieName, buildCsrfToken(context.config.csrfSecret, sessionId), context.config.sessionCookieSecure, context.config.sessionCookieSameSite, expiresAt, false);
}
export async function registerSessionRoutes(app, context) {
    ...
    async function assertLoginAttemptAllowed(identifier) {
        ...
        if (current.failureCount < context.config.loginRateLimitMaxAttempts)
            return;
    ...
    async function findUserByIdentifier(identifier) {
        ...
        return users.find(user => normalizeIdentifier(user.username) === normalized
            || normalizeIdentifier(user.email) === normalized) ?? null;
    }
    async function findPasswordSetupContextByToken(token) {
        ...
        if (!tokenRow)
            throw notFound('Password setup link is invalid');
        const now = context.now();
        if (tokenRow.consumedAt)
            throw badRequest('This password setup link has already been used');
        if (isPasswordSetupTokenExpired(tokenRow.expiresAt, now))
            throw badRequest('This password setup link has expired');
        const [user] = await context.db.select().from(userAccounts).where(eq(userAccounts.userId, tokenRow.userId));
        if (!user || user.status !== 'active')
            throw notFound('Active user account not found for this password setup link');
        ...
        if (!faculty)
            throw notFound('Active faculty profile not found for this password setup link');
    ...
    app.post('/api/session/login', {
        ...
        const grants = sortActiveRoleGrantRows(await context.db.select().from(roleGrants).where(and(eq(roleGrants.facultyId, faculty.facultyId), eq(roleGrants.status, 'active'))));
        if (grants.length === 0) {
            await recordFailedLogin(body.identifier);
    ...
    app.post('/api/session/role-context', {
        ...
        if (!match)
            throw unauthorized('Role grant is not available for this session');
        await context.db.update(sessions).set({
