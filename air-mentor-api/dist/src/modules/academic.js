...
import { z } from 'zod';
import { academicCalendarAuditEvents, academicMeetings, academicRuntimeState, academicTerms, academicTaskPlacements, academicTaskTransitions, academicTasks, alertDecisions, batches, branches, courseOutcomeOverrides, courses, departments, electiveRecommendations, facultyAppointments, facultyCalendarAdminWorkspaces, facultyCalendarWorkspaces, facultyOfferingOwnerships, facultyProfiles, mentorAssignments, offeringAssessmentSchemes, offeringQuestionPapers, reassessmentEvents, riskAssessments, roleGrants, sectionOfferings, simulationStageCheckpoints, simulationStageQueueCases, simulationStageOfferingProjections, simulationStageStudentProjections, simulationRuns, studentAssessmentScores, studentAcademicProfiles, studentAttendanceSnapshots, studentEnrollments, studentInterventions, studentObservedSemesterStates, students, transcriptSubjectResults, transcriptTermResults, userAccounts, institutions, } from '../db/schema.js';
...
import { getProofRiskModelActive, buildHodProofAnalytics, } from '../lib/msruas-proof-control-plane.js';
import { buildObservableFeaturePayload, scoreObservableRiskWithModel, } from '../lib/proof-risk-model.js';
import { buildGraphAwarePrerequisiteSummary, buildMissingGraphAwarePrerequisiteSummary, } from '../lib/graph-summary.js';
import { pickAuthoritativeFirstList, pickAuthoritativeFirstRecord, } from './academic-authoritative-first.js';
import { assertAcademicAccess, evaluateActiveProofRunAccess, evaluateCourseLeaderOfferingManagementAccess, evaluateFacultyContextAccess, evaluateHodOfferingScopeAccess, evaluateHodStudentScopeAccess, evaluateMentorStudentScopeAccess, evaluateOfferingReadRoleAccess, evaluateProofRunSelectionAccess, } from './academic-access.js';
import { DEFAULT_POLICY, resolveBatchCurriculumFeatures, resolveBatchPolicy, resolveBatchStagePolicy, } from './admin-structure.js';
...
const academicRoleCodes = ['COURSE_LEADER', 'MENTOR', 'HOD'];
...
const termTestNodeSchema = z.lazy(() => z.object({
    id: z.string().min(1),
    label: z.string().min(1),
    text: z.string().min(1),
...
function toUiRole(roleCode) {
    if (roleCode === 'COURSE_LEADER')
...
function sortRoleLabels(left, right) {
...
function buildSchemePolicyContext(policy) {
    return {
...
function distributeWeightage(totalWeight, count) {
    if (count <= 0)
        return [];
...
function sanitizeTermTestWeights(weights, totalWeight, maxTermTests) {
    if (maxTermTests <= 0 || totalWeight <= 0)
        return { tt1: 0, tt2: 0 };
    if (maxTermTests === 1) {
...
function canonicalizeSchemeState(input, policy) {
    const policyContext = buildSchemePolicyContext(policy);
    const quizCount = clampInteger(input.quizCount ?? input.quizComponents.length, 0, Math.min(2, policyContext.maxQuizzes), 0);
    const assignmentCount = clampInteger(input.assignmentCount ?? input.assignmentComponents.length, 0, Math.min(2, policyContext.maxAssignments), 0);
    ...
    return {
        finalsMax: (input.finalsMax ?? (policyContext.see > 50 ? 100 : 50)),
...
async function saveAcademicRuntimeState(context, stateKey, payload) {
...
function dedupeRoles(roleCodes) {
    return Array.from(new Set(roleCodes.map(toUiRole).filter((value) => !!value))).sort(sortRoleLabels);
}
function millisToIso(value, fallback) {
...
function normalizeAcademicStudentId(studentId) {
    return studentId.includes('::') ? (studentId.split('::').at(-1) ?? studentId) : studentId;
}
async function resolveStudentShellRun(context, auth, requestedRunId, simulationStageCheckpointId) {
    ...
    if (!run)
        throw notFound('Proof run not found');
    assertAcademicAccess(evaluateActiveProofRunAccess(auth, run.activeFlag === 1));
    return run;
}
async function resolveAcademicStageCheckpoint(context, auth, simulationRunId, simulationStageCheckpointId) {
    ...
    if (auth.activeRoleGrant.roleCode !== 'SYSTEM_ADMIN') {
        ...
        assertAcademicAccess(evaluateActiveProofRunAccess(auth, !!activeRun && activeRun.activeFlag === 1, 'Academic roles may inspect only checkpoints from the active proof run'));
    }
    return checkpoint;
}
async function assertStudentShellScope(context, auth, simulationRunId, studentId, simulationStageCheckpointId) {
    if (auth.activeRoleGrant.roleCode === 'SYSTEM_ADMIN')
        return;
    ...
    if (auth.activeRoleGrant.roleCode === 'HOD') {
        ...
        assertAcademicAccess(evaluateHodStudentScopeAccess(!!analytics.summary.activeRunContext && analytics.summary.activeRunContext.simulationRunId === simulationRunId, 'Student shell is only available for the active HoD proof scope'));
        assertAcademicAccess(evaluateHodStudentScopeAccess(analytics.students.some(row => row.studentId === studentId), 'Student is outside the supervised HoD proof scope'));
        return;
    }
    if (auth.activeRoleGrant.roleCode === 'MENTOR') {
        const [assignment] = await context.db.select().from(mentorAssignments).where(and(eq(mentorAssignments.facultyId, facultyId), eq(mentorAssignments.studentId, studentId)));
        assertAcademicAccess(evaluateMentorStudentScopeAccess(!!assignment && !assignment.effectiveTo, 'Student is outside the active mentor proof scope'));
        return;
    }
    const ownedOfferingIds = new Set((await context.db.select().from(facultyOfferingOwnerships).where(and(eq(facultyOfferingOwnerships.facultyId, facultyId), eq(facultyOfferingOwnerships.status, 'active')))).map(row => row.offeringId));
    if (ownedOfferingIds.size === 0)
        throw forbidden('No owned proof offerings are available for this faculty context');
    const observedRows = await context.db.select().from(studentObservedSemesterStates).where(and(eq(studentObservedSemesterStates.simulationRunId, simulationRunId), eq(studentObservedSemesterStates.studentId, studentId)));
    ...
    assertAcademicAccess(evaluateCourseLeaderOfferingManagementAccess(hasOwnedProofEvidence, 'Student is outside the active course-leader proof scope'));
}
...
function mapCourseOutcomeOverride(row) {
...
function buildDefaultSchemeFromPolicy(policy) {
    const policyContext = buildSchemePolicyContext(policy);
    ...
    return canonicalizeSchemeState({
        finalsMax: (policyContext.see > 50 ? 100 : 50),
...
function normalizeTranscriptCourseKey(courseCode) {
    return courseCode.replace(/\(.*repeat.*\)/i, '').replace(/R$/i, '').trim();
}
function courseFamilyForCode(courseCode) {
...
function computeTranscriptAnalytics(input) {
    ...
    const attemptsByCourseKey = new Map();
    ...
    const selectedAttempts = Array.from(attemptsByCourseKey.values()).map(attempts => {
        if (input.policy.sgpaCgpaRules.repeatedCoursePolicy === 'best-attempt') {
            return [...attempts].sort((left, right) => {
                if (right.gradePoint !== left.gradePoint)
                    return right.gradePoint - left.gradePoint;
    ...
    const trend = latestSgpa - previousSgpa >= 0.25
        ? 'Improving'
        : previousSgpa - latestSgpa >= 0.25
            ? 'Declining'
            : 'Stable';
    const latestBacklogCount = orderedTerms.at(-1)?.backlogCount ?? 0;
    const progressionStatus = (currentCgpa < input.policy.progressionRules.minimumCgpaForPromotion
...
function buildQuestionLeafScoreMap(input) {
    ...
    return Object.fromEntries(input.cells
        .filter(cell => cell.componentType === storageType && !!cell.componentCode)
        .map(cell => [cell.componentCode, cell.score]));
}
function computeStudentOutcomeAttainment(input) {
    ...
    return input.outcomes.map(outcome => {
        const computeComponentAttainment = (leaves, scores) => {
            ...
            if (scoredLeaves.length === 0 || maxScore <= 0)
                return null;
...
function buildStudentReasons(input) {
    const reasons = [];
    if (input.attendancePct < 65)
        reasons.push({ label: `Attendance critically low (${input.attendancePct}%)`, impact: 0.34, feature: 'attendance' });
    else if (input.attendancePct < 75)
        reasons.push({ label: `Attendance below threshold (${input.attendancePct}%)`, impact: 0.22, feature: 'attendance' });
    const termSignals = [
    ...
    termSignals.forEach(signal => {
        ...
        if (pct < 40)
            reasons.push({ label: `Very low ${signal.label} score (${signal.raw}/${signal.max})`, impact: 0.28, feature: signal.feature });
        else if (pct < 60)
            reasons.push({ label: `Below-average ${signal.label} (${signal.raw}/${signal.max})`, impact: 0.16, feature: signal.feature });
    });
    if (input.currentCgpa > 0 && input.currentCgpa < 6)
        reasons.push({ label: `Weak CGPA (${input.currentCgpa.toFixed(2)})`, impact: 0.22, feature: 'cgpa' });
    else if (input.currentCgpa > 0 && input.currentCgpa < 7)
        reasons.push({ label: `Below-average CGPA (${input.currentCgpa.toFixed(2)})`, impact: 0.12, feature: 'cgpa' });
...
function computeRiskFromActiveModelOrPolicy(input) {
    const { attendancePct, currentCgpa, backlogCount, tt1Pct = null, tt2Pct = null, quizPct = null, assignmentPct = null, seePct = null, weakCoCount = 0, policy, activeModel = null, semesterProgress = 1, prerequisiteSummary, } = input;
...
function resolveCourseOutcomesForOffering(input) {
    ...
    const scopeChain = [
        ...
        ...(input.batchId ? [{ scopeType: 'batch', scopeId: input.batchId }] : []),
...
function buildDefaultQuestionPaper(kind, outcomes) {
...
function collectBlueprintOutcomeIds(nodes) {
    const collected = new Set();
    const visit = (entries) => {
        for (const entry of entries) {
            for (const coId of entry.cos)
                collected.add(coId);
...
function validateSchemeAgainstPolicy(input, policy) {
...
function validateQuestionPaperBlueprint(kind, blueprint, allowedOutcomeIds) {
...
async function getOfferingContext(context, offeringId) {
    ...
    if (!course)
        throw notFound('Course not found for offering');
    if (!term)
        throw notFound('Academic term not found for offering');
    if (!branch)
        throw notFound('Branch not found for offering');
    const [department] = await context.db.select().from(departments).where(eq(departments.departmentId, branch.departmentId));
    if (!department)
        throw notFound('Department not found for offering');
    return { offering, course, term, branch, department };
}
function stagePolicyForOffering(batchPolicy) {
    return batchPolicy?.effectivePolicy ?? DEFAULT_STAGE_POLICY;
}
function offeringStageSnapshot(offering, policy) {
...
async function buildOfferingStageEligibility(context, offeringId) {
    ...
    const runtimeLockByOffering = await getAcademicRuntimeState(context, 'lockByOffering');
    const runtimeLock = runtimeLockByOffering[offeringId] ?? {};
    const activeEnrollments = await context.db.select().from(studentEnrollments).where(and(eq(studentEnrollments.termId, offering.termId), eq(studentEnrollments.sectionCode, offering.sectionCode), eq(studentEnrollments.academicStatus, 'active')));
    ...
    const hasScoresFor = (types) => {
        const students = new Set(assessmentRows
    ...
    const openQueueCases = activeRun
        ? queueCaseRows.filter(row => row.simulationRunId === activeRun.simulationRunId
            && row.status !== 'resolved'
            && row.status !== 'idle')
        : [];
    const activeOwnerships = ownershipRows.filter(row => row.status === 'active');
    const explicitScheme = schemeRows[0]
        ? canonicalizeSchemeState(schemeStateSchema.parse(parseJson(schemeRows[0].schemeJson, {})), resolvedPolicy?.effectivePolicy ?? DEFAULT_POLICY)
        : null;
...
function mockStudentIdentity(index) {
...
async function assertSingleActiveOfferingOwner(context, offeringId, facultyId, excludeOwnershipId) {
    const activeOwnerships = await context.db
        .select()
        .from(facultyOfferingOwnerships)
        .where(and(eq(facultyOfferingOwnerships.offeringId, offeringId), eq(facultyOfferingOwnerships.status, 'active')));
...
async function assertStudentEnrolledInOffering(context, offering, studentId) {
    const normalizedStudentId = normalizeAcademicStudentId(studentId);
    const [enrollment] = await context.db
        .select()
        .from(studentEnrollments)
        .where(and(eq(studentEnrollments.studentId, normalizedStudentId), eq(studentEnrollments.termId, offering.termId), eq(studentEnrollments.sectionCode, offering.sectionCode), eq(studentEnrollments.academicStatus, 'active')));
    if (!enrollment) {
...
function flattenTermTestLeaves(nodes) {
...
async function assertCourseLeaderCanManageOffering(context, facultyId, offeringId) {
    const [ownership] = await context.db
        .select()
        .from(facultyOfferingOwnerships)
        .where(and(eq(facultyOfferingOwnerships.facultyId, facultyId), eq(facultyOfferingOwnerships.offeringId, offeringId), eq(facultyOfferingOwnerships.status, 'active')));
    assertAcademicAccess(evaluateCourseLeaderOfferingManagementAccess(!!ownership && isLeaderLikeOwnershipRole(ownership.ownershipRole)));
    return ownership;
}
async function assertViewerCanReadOffering(context, auth, offeringId) {
    if (auth.activeRoleGrant.roleCode === 'SYSTEM_ADMIN')
        return;
    assertAcademicAccess(evaluateFacultyContextAccess(auth));
    const facultyId = auth.facultyId;
    ...
    const appointments = await context.db
        ...
        .where(and(eq(facultyAppointments.facultyId, facultyId), eq(facultyAppointments.status, 'active')));
    const scopedDepartmentIds = new Set(appointments.map(row => row.departmentId));
    const explicitBranchIds = new Set(appointments.map(row => row.branchId).filter((value) => !!value));
    const termInScope = appointments.some(row => row.branchId === offering.branchId || row.departmentId === branch.departmentId);
    assertAcademicAccess(evaluateHodOfferingScopeAccess(termInScope || scopedDepartmentIds.has(branch.departmentId) || explicitBranchIds.has(offering.branchId)));
}
async function assertViewerCanManageTask(context, auth, task) {
    assertAcademicAccess(evaluateFacultyContextAccess(auth));
    const facultyId = auth.facultyId;
    ...
    if (auth.activeRoleGrant.roleCode === 'MENTOR') {
        const [assignment] = await context.db
            ...
            .where(and(eq(mentorAssignments.facultyId, facultyId), eq(mentorAssignments.studentId, normalizedStudentId)));
...
function validateMeetingWindow(startMinutes, endMinutes) {
...
async function assertViewerCanSuperviseStudent(input) {
    ...
    const facultyId = input.auth.facultyId;
    ...
    if (input.auth.activeRoleGrant.roleCode === 'COURSE_LEADER') {
        ...
        const activeEnrollments = await input.context.db
            .select()
            .from(studentEnrollments)
            .where(and(eq(studentEnrollments.studentId, normalizedStudentId), eq(studentEnrollments.academicStatus, 'active')));
        const ownedOfferingRows = await input.context.db
            .select()
            .from(facultyOfferingOwnerships)
            .where(and(eq(facultyOfferingOwnerships.facultyId, facultyId), eq(facultyOfferingOwnerships.status, 'active')));
        const ownedOfferingIds = new Set(ownedOfferingRows
        ...
        assertAcademicAccess(evaluateCourseLeaderOfferingManagementAccess(!!matchingOffering, 'This course leader does not supervise the selected student'));
        return { student, studentId: normalizedStudentId };
    }
    if (input.auth.activeRoleGrant.roleCode === 'HOD') {
        ...
        const appointments = await input.context.db
            ...
            .where(and(eq(facultyAppointments.facultyId, facultyId), eq(facultyAppointments.status, 'active')));
        const scopedDepartmentIds = new Set(appointments.map(row => row.departmentId));
        const scopedBranchIds = new Set(appointments.map(row => row.branchId).filter((value) => !!value));
...
function proofResolutionRecoveryState(outcome) {
...
async function resolveProofReassessmentAccess(input) {
    ...
    const [run] = risk.simulationRunId
        ...
        : [];
    if (!run)
        throw notFound('Proof reassessment run context not found');
...
function mapAcademicTaskRow(row, transitions) {
...
function mapTaskTransitionRow(row) {
...
function mapTaskPlacementRow(row) {
...
function mapFacultyCalendarWorkspaceRow(row) {
...
function mapCalendarAuditEventRow(row) {
...
async function buildAcademicMeetingResponse(context, row) {
...
function classBlocksCanOverlap(left, right) {
...
async function validateFacultyCalendarTemplate(context, facultyId, template) {
    ...
    const ownershipRows = await context.db
        ...
        .from(facultyOfferingOwnerships)
        .where(and(eq(facultyOfferingOwnerships.facultyId, facultyId), eq(facultyOfferingOwnerships.status, 'active')));
    const ownedOfferingIds = new Set(ownershipRows
        .filter(row => isLeaderLikeOwnershipRole(row.ownershipRole))
        .map(row => row.offeringId));
    ...
    for (let index = 0; index < template.classBlocks.length; index += 1) {
        ...
        for (let compareIndex = index + 1; compareIndex < template.classBlocks.length; compareIndex += 1) {
            ...
            if (!classBlocksCanOverlap(left, right))
                continue;
...
async function getEditableCalendarWindowStatus(context, facultyId) {
    ...
    const runtimeWorkspace = workspaceFromTable
        ? null
        : await getAcademicRuntimeState(context, 'adminCalendarByFacultyId');
...
function normalizePlaybackRiskBand(value) {
    return value === 'High' || value === 'Medium' || value === 'Low' ? value : 'Low';
}
function toPlaybackReasonRows(attentionAreas, recommendedAction) {
...
async function buildAcademicBootstrap(context, viewer = {}) {
    const runtimeEntries = await Promise.all(runtimeStateKeys.map(async (stateKey) => {
        return [stateKey, await getAcademicRuntimeState(context, stateKey)];
    }));
    const runtime = Object.fromEntries(runtimeEntries);
    const [courseRows, termRows, branchRows, departmentRows, offeringRows, ownershipRows, facultyRows, appointmentRows, userRows, roleGrantRows, studentRows, profileRows, enrollmentRows, mentorRows, attendanceRows, assessmentRows, interventionRows, transcriptTermRows, transcriptSubjectRows, courseOutcomeOverrideRows, schemeRows, questionPaperRows, riskAssessmentRows, electiveRecommendationRows, academicTaskRows, academicTaskTransitionRows, academicTaskPlacementRows, facultyCalendarWorkspaceRows, facultyCalendarAdminWorkspaceRows, academicCalendarAuditRows, academicMeetingRows, runRows, stageCheckpointRow, stageOfferingProjectionRows, stageStudentProjectionRows,] = await Promise.all([
    ...
    const proofScopeRun = stageCheckpointRow
        ...
        : selectedActiveRun;
    const proofBatchIds = Array.from(new Set([proofScopeRun?.batchId ?? null].filter((value) => !!value)));
    ...
    const scopedTermIds = new Set(termRows
    ...
    const scopedDepartmentIds = new Set(Array.from(scopedBranchIds)
    ...
    const playbackObservedStateRows = stageCheckpointRow
        ...
        : [];
    const playbackObservedSummaryByStudentId = new Map();
    ...
    const playbackStudentOverlayByOfferingStudent = new Map();
    for (const row of stageStudentProjectionRows) {
        ...
        playbackStudentOverlayByOfferingStudent.set(`${row.offeringId}::${row.studentId}`, {
            ...
            attentionAreas: Array.isArray(currentStatus.attentionAreas)
                ? currentStatus.attentionAreas.filter((value) => typeof value === 'string').slice(0, 4)
                : [],
    ...
    const activeEnrollmentByStudentId = new Map();
    const primaryAppointmentByFacultyId = new Map();
    ...
    const activeMentorAssignmentByStudentId = new Map();
    ...
    const latestAttendanceByStudentOffering = new Map();
    const attendanceSourcePriority = (source) => {
        if (source === 'teacher-workspace')
            return 2;
        if (source.startsWith('proof-run:'))
            return 1;
        return 0;
    };
    ...
    const latestAssessmentsByStudentOffering = new Map();
    const latestAssessmentCellsByStudentOffering = new Map();
    const latestAssessmentCellByCompositeKey = new Map();
    ...
    const latestRiskAssessmentByStudentOffering = new Map();
    ...
    const latestElectiveRecommendationByStudentId = new Map();
    ...
    const latestTranscriptTermByStudentAndTerm = new Map();
    ...
    const transcriptTermsByStudentId = new Map();
    ...
    const transcriptSubjectsByTermResultId = new Map();
    ...
    const transcriptTermResultById = new Map(transcriptTermRows.map(row => [row.transcriptTermResultId, row]));
    const latestTranscriptSubjectByStudentCourseCode = new Map();
    for (const row of transcriptSubjectRows) {
    ...
    const transcriptSubjectHistoryByStudentCourseCode = new Map();
    ...
    const resolvedPolicyByBatchId = new Map();
    const resolvedStagePolicyByBatchId = new Map();
    const batchIds = Array.from(new Set(termRows.map(row => row.batchId).filter((value) => !!value)));
    const [resolvedPolicies, resolvedStagePolicies] = await Promise.all([
        Promise.all(batchIds.map(async (batchId) => {
            ...
            return [batchId, resolved.effectivePolicy];
        })),
        Promise.all(batchIds.map(async (batchId) => {
    ...
    const activeRiskModelByBatchId = new Map();
    ...
    const studentTranscriptAnalyticsByStudentId = new Map();
    ...
    const courseOutcomeOverridesByCourseId = new Map();
    ...
    const rawSchemeByOfferingId = new Map();
    for (const row of schemeRows) {
        const parsed = schemeStateSchema.safeParse(parseJson(row.schemeJson, {}));
        if (parsed.success)
            rawSchemeByOfferingId.set(row.offeringId, parsed.data);
    }
    const questionPapersByOfferingId = new Map();
    for (const row of questionPaperRows) {
        if (row.kind !== 'tt1' && row.kind !== 'tt2')
            continue;
    ...
    const taskTransitionsByTaskId = new Map();
    ...
    const authoritativePlacementsByTaskId = new Map();
    for (const row of academicTaskPlacementRows) {
        authoritativePlacementsByTaskId.set(row.taskId, mapTaskPlacementRow(row));
    }
    const facultyCalendarTemplateByFacultyId = new Map();
    for (const row of facultyCalendarWorkspaceRows) {
        const parsed = mapFacultyCalendarWorkspaceRow(row);
        if (parsed)
            facultyCalendarTemplateByFacultyId.set(row.facultyId, parsed);
    }
    const facultyCalendarAdminWorkspaceByFacultyId = new Map();
    for (const row of facultyCalendarAdminWorkspaceRows) {
        const parsed = mapFacultyCalendarAdminWorkspaceRow(row);
        if (parsed)
            facultyCalendarAdminWorkspaceByFacultyId.set(row.facultyId, parsed);
    }
    const calendarAuditByFacultyId = new Map();
    ...
    const academicOfferings = scopedOfferingRows.map(offeringRow => {
    ...
    const resolvedPolicyByOfferingId = new Map();
    const resolvedCourseOutcomesByOfferingId = new Map();
    const resolvedSchemesByOfferingId = new Map();
    const resolvedQuestionPapersByOfferingId = new Map();
    for (const offeringRow of scopedOfferingRows) {
        ...
        resolvedSchemesByOfferingId.set(offeringRow.offeringId, rawSchemeByOfferingId.has(offeringRow.offeringId)
            ? canonicalizeSchemeState(rawSchemeByOfferingId.get(offeringRow.offeringId), policy)
            : buildDefaultSchemeFromPolicy(policy));
        resolvedQuestionPapersByOfferingId.set(offeringRow.offeringId, {
    ...
    const meetingEntriesByStudentId = new Map();
        ...
        const nextStudents = sectionEnrollments.map(enrollment => {
            ...
            const risk = persistedRisk
                ? {
                    ...
                    riskBand: persistedRisk.riskBand,
                    riskCompleteness: prerequisiteSummary.featureCompleteness,
                    featureCompleteness: prerequisiteSummary.featureCompleteness,
                    featureProvenance: prerequisiteSummary.featureProvenance,
                }
    ...
    const studentHistoryByUsn = Object.fromEntries(scopedStudentRows.map(student => {
        ...
        return [student.usn, buildStudentHistoryRecord({
                ...
                electiveRecommendation: (() => {
                    ...
                    if (!recommendation)
                        return null;
                    const rationale = parseJson(recommendation.rationaleJson, { summary: '' });
                    const alternatives = parseJson(recommendation.alternativesJson, []);
                    return {
        ...
        const courseRisks = enrollment
            ? academicOfferings
                ...
                return {
                    ...
                    band: matchingProjection?.riskBand ?? 'Low',
                    stage: item.stage,
                };
            })
            : [];
    ...
    const offeringIdsByFacultyId = new Map();
    ...
    const menteeIdsByFacultyId = new Map();
    ...
    const grantIntersectsProofScope = (grant) => {
        if (!proofScopeActive)
            return true;
        if (grant.scopeType === 'institution')
            return true;
        if (grant.scopeType === 'academic-faculty')
            return grant.scopeId === grant.facultyId;
        if (grant.scopeType === 'department')
            return scopedDepartmentIds.has(grant.scopeId);
        if (grant.scopeType === 'branch')
            return scopedBranchIds.has(grant.scopeId);
        if (grant.scopeType === 'batch')
            return proofBatchIds.includes(grant.scopeId);
        if (grant.scopeType === 'offering')
            return scopedOfferingIds.has(grant.scopeId);
        return true;
    };
    const faculty = facultyRows
        ...
        const courseCodes = Array.from(new Set(offeringIds.map(offeringId => offeringCodeById[offeringId]).filter((value) => !!value)));
        ...
        const allowedRoles = dedupeRoles(proofScopedGrants.map(grant => grant.roleCode)).filter(roleLabel => {
            if (roleLabel === 'Course Leader')
                return offeringIds.length > 0;
            if (roleLabel === 'Mentor')
                return nextMenteeIds.length > 0;
            if (roleLabel === 'HoD')
                return proofScopedGrants.some(grant => toUiRole(grant.roleCode) === 'HoD');
            return true;
        });
    ...
    const yearGroups = ['1st Year', '2nd Year', '3rd Year', '4th Year']
        .map(year => ({
        year,
        color: { '1st Year': '#f59e0b', '2nd Year': '#6366f1', '3rd Year': '#10b981', '4th Year': '#ec4899' }[year] ?? '#8892a4',
    ...
    const subjectRuns = Object.values(academicOfferings.reduce((acc, offering) => {
    ...
    if (viewerAccount && viewerRole === 'Course Leader') {
        ...
        visibleStudentIds = new Set(Array.from(visibleOfferingIds).flatMap(offeringId => {
        ...
        const explicitBranchIds = new Set(hodAppointments.map(row => row.branchId).filter((value) => !!value));
        const hodBranchIds = new Set(branchRows
        ...
        visibleStudentIds = new Set(scopedEnrollmentRows
            .filter(row => hodBranchIds.has(row.branchId) || hodTermIds.has(row.termId))
            .map(row => row.studentId));
        visibleFacultyIds = new Set(appointmentRows
    ...
    const playbackOfferingOverlayByOfferingId = new Map(stageOfferingProjectionRows.map(row => [row.offeringId, row]).filter((entry) => !!entry[0]));
    ...
    const filteredStudentsByOffering = Object.fromEntries(Object.entries(studentsByOffering)
    ...
    const filteredStudentHistoryByUsn = Object.fromEntries(Object.entries(studentHistoryByUsn).filter(([usn]) => visibleUsns.has(usn)));
    ...
    const filteredCoAttainmentByOffering = Object.fromEntries(Object.entries(coAttainmentByOffering).filter(([offeringId]) => visibleOfferingIds.has(offeringId)));
    const filteredMeetings = authoritativeMeetings.filter(meeting => visibleStudentIds.has(meeting.studentId));
    const authoritativeStudentPatches = Object.fromEntries(Array.from(new Set([
    ...
    const authoritativeLockByOffering = Object.fromEntries(filteredOfferings.map(offering => {
        const runtimeLock = (runtime.lockByOffering ?? {})[offering.offId] ?? {};
    ...
    const visibleTasks = sourceTasks.filter(task => {
        if (viewerRole && task.assignedTo !== viewerRole)
            return false;
        if (visibleStudentIds.size > 0 && !visibleStudentIds.has(task.studentId))
            return false;
        if (visibleOfferingIds.size > 0 && !visibleOfferingIds.has(task.offeringId))
            return false;
        return true;
    });
    const visibleTaskIds = new Set(visibleTasks.map(task => task.id));
    const runtimeTaskPlacements = Object.fromEntries(Object.entries(runtime.taskPlacements ?? {})
        .flatMap(([taskId, placement]) => {
        const parsed = taskPlacementSchema.safeParse(placement);
        return parsed.success ? [[taskId, parsed.data]] : [];
    }));
    const resolvedTasksFromAuthoritativeRows = Object.fromEntries(visibleTasks
    ...
    const runtimeCalendarAuditEvents = (runtime.calendarAudit ?? [])
    ...
    const currentFacultyTemplate = viewerAccount
        ? facultyCalendarTemplateByFacultyId.get(viewerAccount.facultyId)
            ?? facultyCalendarTemplateSchema.safeParse(runtime.timetableByFacultyId?.[viewerAccount.facultyId]).data
        : null;
    ...
    const mergedCalendarAuditEvents = viewerAccount
        ? pickAuthoritativeFirstList(currentFacultyAuditEvents, runtimeCalendarAuditEvents.filter(event => event.facultyId === viewerAccount.facultyId))
        : [];
    const filteredRuntime = {
        ...runtime,
        studentPatches: Object.keys(authoritativeStudentPatches).length > 0
            ? Object.fromEntries(Object.entries(authoritativeStudentPatches).filter(([key]) => {
        ...
        resolvedTasks: authoritativeTasks.length > 0
            ...
            : Object.fromEntries(Object.entries(runtime.resolvedTasks).filter(([taskId]) => visibleTaskIds.has(taskId))),
        ...
        adminCalendarByFacultyId: viewerAccount
            ? (() => {
                ...
                if (workspace)
                    return { [viewerAccount.facultyId]: workspace };
                return Object.fromEntries(Object.entries(runtime.adminCalendarByFacultyId).filter(([facultyId]) => facultyId === viewerAccount.facultyId));
            })()
            : {},
...
async function buildPublicFacultyList(context) {
...
export async function registerAcademicRoutes(app, context) {
