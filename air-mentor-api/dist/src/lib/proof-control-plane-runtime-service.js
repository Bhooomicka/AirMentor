...
import { alertAcknowledgements, alertDecisions, alertOutcomes, facultyOfferingOwnerships, mentorAssignments, reassessmentEvents, reassessmentResolutions, riskAssessments, riskEvidenceSnapshots, riskOverrides, roleGrants, simulationResetSnapshots, simulationRuns, studentObservedSemesterStates, teacherAllocations, teacherLoadProfiles, } from '../db/schema.js';
...
import { buildObservableFeaturePayload, featureHash, RISK_FEATURE_SCHEMA_VERSION, scoreObservableRiskWithModel, } from './proof-risk-model.js';
import { governProofQueueStage, } from './proof-queue-governance.js';
export async function restoreProofSimulationSnapshot(db, input, deps) {
    ...
    const snapshot = input.simulationResetSnapshotId
        ...
        : snapshotRows[0];
    if (!snapshot)
        throw new Error('Simulation snapshot not found');
...
export async function recomputeObservedOnlyRisk(db, input, deps) {
    const [run] = await db.select().from(simulationRuns).where(eq(simulationRuns.simulationRunId, input.simulationRunId));
    if (!run)
        throw new Error('Simulation run not found');
    ...
    const latestResolutionByStudentOffering = new Map();
    ...
    const currentSemesterNumber = Math.max(run.semesterEnd, observedRows.reduce((max, row) => Math.max(max, row.semesterNumber), 0));
    const latestHistoricalByStudent = new Map();
    observedRows
    ...
    const stageCloseEvidenceByStudentOffering = new Map();
    runEvidenceRows
    ...
    const liveStageSourceKey = (studentId, offeringId, courseCode) => `${studentId}::${currentSemesterNumber}::${offeringId}::${courseCode}`;
    const liveCaseKey = (studentId) => `${studentId}::${currentSemesterNumber}`;
    const liveQueueCaseId = (studentId, stageKey) => deps.buildDeterministicId('runtime_queue_case', [input.simulationRunId, studentId, currentSemesterNumber, stageKey]);
    ...
    const supervisedSectionCount = new Set(teacherAllocationRows
        ...
        .map(row => row.sectionCode)).size;
    const facultyBudgetByKey = new Map();
    currentSemesterLoads.forEach(row => {
        const overloadPenalty = overloadPenaltyBySemesterFaculty.get(row.facultyId) ?? 0;
        const ownedOfferingCount = teacherAllocationRows.filter(allocation => allocation.semesterNumber === currentSemesterNumber
    ...
    const runtimeFacultyAssignment = (studentId, offeringId, assignedRole) => {
    ...
    const liveStageKeyForPayload = (payload) => {
        if (payload.seePct != null)
            return 'post-see';
        if (payload.assignmentPct != null)
            return 'post-assignments';
        if (payload.tt2Pct != null)
            return 'post-tt2';
        if (payload.tt1Pct != null)
            return 'post-tt1';
        return 'pre-tt1';
    };
    const rawCurrentSemesterRows = observedRows.filter(row => row.semesterNumber === currentSemesterNumber);
    const currentSemesterRowsByStudentOffering = new Map();
    ...
    const currentSemesterSectionStudentCountByKey = new Map();
    ...
    const sectionRiskRateBySemesterSection = new Map();
    const sectionRiskRateSeed = new Map();
    ...
    const riskRows = [];
    const activeEvidenceRows = [];
    const reassessmentRows = [];
    const alertRows = [];
    const alertOutcomeRows = [];
    const runtimeQueueCandidates = [];
    for (const row of currentSemesterRows) {
        const payload = parseObservedStateRow(row);
        ...
        const stageSourceRefs = stageEvidence?.sourceRefs;
        ...
        const featurePayload = stageEvidence?.featurePayload ?? buildObservableFeaturePayload({
            attendancePct: Number(payload.attendancePct ?? 0),
            attendanceHistory: parseJson(JSON.stringify(payload.attendanceHistory ?? []), []),
        ...
        const inference = scoreObservableRiskWithModel({
            ...
            attendanceHistoryRiskCount: Array.isArray(payload.attendanceHistory)
                ? payload.attendanceHistory.filter(entry => Number(entry.attendancePct ?? 0) < input.policy.attendanceRules.minimumRequiredPercent).length
                : 0,
            questionWeaknessCount: Number(payload.questionEvidenceSummary?.weakQuestionCount ?? 0),
            interventionResponseScore,
            policy: input.policy,
            featurePayload,
        ...
        const assignment = runtimeFacultyAssignment(row.studentId, offeringId, monitoring.queueOwnerRole);
        runtimeQueueCandidates.push({
            ...
            assignedRole: monitoring.queueOwnerRole,
    ...
    const liveCaseStateByKey = new Map();
    deps.PLAYBACK_STAGE_DEFS.forEach(stage => {
        ...
        const primaryCandidateByCaseKey = new Map();
        stageCandidates.forEach(candidate => {
            const decision = decisionsByCaseKey.get(candidate.caseKey);
            if (!decision || decision.primarySourceKey !== candidate.sourceKey)
                return;
    ...
    if (activeEvidenceRows.length > 0)
        await deps.insertRowsInChunks(db, riskEvidenceSnapshots, activeEvidenceRows);
    ...
    if (alertRows.length > 0)
        await db.insert(alertDecisions).values(alertRows);
    if (alertOutcomeRows.length > 0)
