...
export function scenarioFamilyForSeed(seed) {
...
function featureVectorFromPayload(payload) {
...
function chooseCalibration(headKey, validationRows, testRows, support) {
    const candidates = [];
...
function writeFeatureVectorToBuffer(payload, buffer, offset) {
    const prerequisiteDepth = Math.max(0, safeNumber(payload.prerequisiteChainDepth));
...
class ProofRiskDatasetBuilder {
    ...
    constructor(runMetadataById, manifest) {
...
export function createProofRiskModelTrainingBuilder(options) {
    return new ProofRiskDatasetBuilder(normalizeMetadataMap(options?.runMetadataById), options?.manifest ?? PROOF_CORPUS_MANIFEST);
}
...
function fitLogisticHeadCompact(dataset, headKey) {
...
function fitStumpHeadCompact(dataset, headKey) {
    const sampledTrainRows = sampleTrainingIndices(dataset, headKey);
...
function trainCompactProofRiskModel(dataset, now, correlationsAccumulator) {
