...
export function proofQueueActionableRateLimitForStage(stageKey) {
...
function riskBandWeight(riskBand) {
    if (riskBand === 'High')
        return 2;
    if (riskBand === 'Medium')
        return 1;
...
export function governProofQueueStage(input) {
    const priorCaseStateByKey = input.priorCaseStateByKey ?? new Map();
    ...
    input.candidates.forEach(candidate => {
        ...
        if (eligibility.openEligible)
            group.openCandidates.push(candidate);
        if (eligibility.watchEligible)
            group.watchCandidates.push(candidate);
        group.allCandidates.push(candidate);
        candidateGroups.set(candidate.caseKey, group);
    });
    const openCaseCandidates = [];
    const watchCaseCandidates = new Map();
    ...
    const sectionUsageByKey = new Map();
    const facultyUsageByKey = new Map();
    const admittedCaseKeys = new Set();
    ...
    openCaseCandidates
        ...
        if (currentSectionUsage >= sectionLimit)
            return;
        if (currentFacultyUsage >= facultyBudget)
            return;
        admittedCaseKeys.add(candidate.caseKey);
        sectionUsageByKey.set(sectionKey, currentSectionUsage + 1);
        if (facultyKey)
            facultyUsageByKey.set(facultyKey, currentFacultyUsage + 1);
    ...
    candidateGroups.forEach((group, caseKey) => {
        if (admittedCaseKeys.has(caseKey))
            return;
