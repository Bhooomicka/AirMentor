...
export function inferObservableDrivers(input) {
    const drivers = [];
    ...
    const termSignals = [
        { key: 'tt1', label: 'TT1', pct: input.tt1Pct ?? null },
        { key: 'tt2', label: 'TT2', pct: input.tt2Pct ?? null },
        { key: 'see', label: 'SEE', pct: input.seePct ?? null },
    ];
...
export function inferObservableRisk(input) {
    const drivers = inferObservableDrivers(input);
    let riskProb = 0.08;
    for (const driver of drivers)
        riskProb += driver.impact;
    const bounded = Math.max(0.05, Math.min(0.95, roundToTwo(riskProb)));
    const riskBand = bounded >= 0.7 ? 'High' : bounded >= 0.35 ? 'Medium' : 'Low';
    return {
        riskProb: bounded,
        riskBand,
