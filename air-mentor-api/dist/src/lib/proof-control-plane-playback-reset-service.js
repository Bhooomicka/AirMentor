...
import { riskEvidenceSnapshots, simulationStageCheckpoints, simulationStageOfferingProjections, simulationStageQueueCases, simulationStageQueueProjections, simulationStageStudentProjections, studentAgentCards, studentAgentMessages, studentAgentSessions, } from '../db/schema.js';
export async function resetPlaybackStageArtifacts(db, simulationRunId) {
    ...
    if (stageEvidenceIds.length > 0) {
        await db.delete(riskEvidenceSnapshots).where(and(eq(riskEvidenceSnapshots.simulationRunId, simulationRunId), isNotNull(riskEvidenceSnapshots.simulationStageCheckpointId)));
    }
