...
import { alertAcknowledgements, alertDecisions, batches, courses, curriculumImportVersions, curriculumValidationResults, facultyProfiles, officialCodeCrosswalks, operationalTelemetryEvents, reassessmentEvents, reassessmentResolutions, riskAssessments, sectionOfferings, simulationLifecycleAudits, simulationResetSnapshots, simulationRuns, simulationQuestionTemplates, simulationStageCheckpoints, simulationStageOfferingProjections, simulationStageQueueProjections, simulationStageStudentProjections, studentBehaviorProfiles, studentCoStates, studentInterventionResponseStates, studentQuestionResults, students, studentTopicStates, teacherLoadProfiles, worldContextSnapshots, } from '../db/schema.js';
import { notFound } from './http-errors.js';
import { parseJson } from './json.js';
import { buildCheckpointReadinessDiagnostics, buildProofQueueDiagnostics, buildProofWorkerDiagnostics, decorateProofRunsWithOperationalDiagnostics, } from './proof-control-plane-dashboard-service.js';
...
async function resolveProofCheckpointForRun(db, simulationRunId, simulationStageCheckpointId) {
    const [checkpoint] = await db.select().from(simulationStageCheckpoints).where(eq(simulationStageCheckpoints.simulationStageCheckpointId, simulationStageCheckpointId));
    if (!checkpoint)
...
export async function listProofRunCheckpoints(db, input, deps) {
    const { parseProofCheckpointSummary, withProofPlaybackGate } = deps;
    const rows = await db.select().from(simulationStageCheckpoints).where(eq(simulationStageCheckpoints.simulationRunId, input.simulationRunId)).orderBy(asc(simulationStageCheckpoints.semesterNumber), asc(simulationStageCheckpoints.stageOrder));
    return withProofPlaybackGate(rows.map(parseProofCheckpointSummary));
}
export async function getProofRunCheckpointDetail(db, input, deps) {
    const { parseProofCheckpointSummary, queueStatusPriority, withProofPlaybackGate } = deps;
    const checkpoint = await resolveProofCheckpointForRun(db, input.simulationRunId, input.simulationStageCheckpointId);
    ...
    const orderedCheckpointRows = await db.select().from(simulationStageCheckpoints).where(eq(simulationStageCheckpoints.simulationRunId, input.simulationRunId)).orderBy(asc(simulationStageCheckpoints.semesterNumber), asc(simulationStageCheckpoints.stageOrder));
...
export async function getProofRunCheckpointStudentDetail(db, input, deps) {
    const { parseProofCheckpointSummary, withProofPlaybackGate } = deps;
    const [checkpoint, student, projectionRows] = await Promise.all([
        ...
        db.select().from(simulationStageStudentProjections).where(and(eq(simulationStageStudentProjections.simulationRunId, input.simulationRunId), eq(simulationStageStudentProjections.simulationStageCheckpointId, input.simulationStageCheckpointId), eq(simulationStageStudentProjections.studentId, input.studentId))),
    ]);
    if (!student)
    ...
    return {
        ...
        projections: projectionRows
            ...
            riskChangeFromPreviousCheckpointScaled: Number((parseJson(row.projectionJson, {}).riskChangeFromPreviousCheckpointScaled) ?? 0),
            counterfactualLiftScaled: Number((parseJson(row.projectionJson, {}).counterfactualLiftScaled) ?? (row.noActionRiskProbScaled ?? row.riskProbScaled) - row.riskProbScaled),
            projection: parseJson(row.projectionJson, {}),
        })),
    };
}
export async function buildProofBatchDashboard(db, batchId, deps) {
    ...
    const [importRows, validationRows, crosswalkRows, runRows, lifecycleRows, resolutionRows, acknowledgementRows, recentOperationalEventRows, offeringRows, courseRows,] = await Promise.all([
    ...
    const proofRunStatusRank = (status) => {
        switch (status) {
            case 'active':
                return 0;
            case 'running':
    ...
    const activeRun = pickMostRecentActiveRun(runRows
    ...
    const [activeSnapshots, activeLoads, activeBehaviorProfiles, activeTopicStates, activeCoStates, activeQuestionTemplates, activeQuestionResults, activeInterventionResponses, activeWorldContexts, activeStageCheckpoints, activeStageQueueRows, activeRiskRows,] = activeRunId
        ? await Promise.all([
            db.select().from(simulationResetSnapshots).where(and(eq(simulationResetSnapshots.batchId, batchId), eq(simulationResetSnapshots.simulationRunId, activeRunId))),
            ...
            db.select().from(simulationStageCheckpoints).where(eq(simulationStageCheckpoints.simulationRunId, activeRunId)).orderBy(asc(simulationStageCheckpoints.semesterNumber), asc(simulationStageCheckpoints.stageOrder)),
    ...
    const facultyIds = Array.from(new Set([
        ...
        ...activeStageQueueRows.map(row => row.assignedFacultyId).filter((value) => typeof value === 'string' && value.length > 0),
        ...activeReassessments.map(row => row.assignedFacultyId).filter((value) => typeof value === 'string' && value.length > 0),
        ...lifecycleRows.map(row => row.createdByFacultyId).filter((value) => typeof value === 'string' && value.length > 0),
    ]));
    ...
    const activeQueue = activeReassessments
        ...
        return {
            ...
            sourceKind: 'runtime-reassessment',
    ...
    const playbackQueue = (() => {
        if (activeQueue.length > 0)
            return activeQueue;
        if (activeStageQueueRows.length === 0)
            return activeQueue;
    ...
    const proofRuns = decorateProofRunsWithOperationalDiagnostics(runRows
        ...
        progress: parseJson(row.progressJson, null),
    ...
    return {
        imports: importRows
            ...
            createdAt: row.createdAt,
            certificate: parseJson(row.completenessCertificateJson, {}),
        })),
        latestValidation: latestValidationRow
            ? {
                ...
                summary: parseJson(latestValidationRow.summaryJson, {}),
            }
            : null,
        ...
        activeRunDetail: activeRun ? {
            ...
            progress: parseJson(activeRun.progressJson, null),
            monitoringSummary: {
            ...
            teacherAllocationLoad: activeLoads.map(load => ({
                ...
                permissions: parseJson(load.permissionsJson, []),
            })),
            ...
            snapshots: activeSnapshots.map(snapshot => ({
                ...
                payload: parseJson(snapshot.snapshotJson, {}),
            })),
