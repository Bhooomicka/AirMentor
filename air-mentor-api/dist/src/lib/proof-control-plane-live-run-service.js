...
import { academicTerms, batches, courses, curriculumCourses, facultyOfferingOwnerships, sectionOfferings, simulationResetSnapshots, simulationRuns, studentAcademicProfiles, studentAssessmentScores, studentAttendanceSnapshots, studentEnrollments, studentObservedSemesterStates, students, teacherAllocations, teacherLoadProfiles, transcriptSubjectResults, transcriptTermResults, } from '../db/schema.js';
function buildLiveAttendanceHistory(input) {
    ...
    return checkpoints.map(checkpoint => {
        ...
        const scaledPresent = Math.min(scaledTotal, safeTotalClasses > 0
...
function pctFromScoredComponents(rows, componentTypes, deps) {
...
function latestRowByKey(rows, keyOf) {
...
export async function startLiveBatchProofSimulationRun(db, input, deps) {
    ...
    const [termRows, curriculumRows, courseRows, offeringRows, enrollmentRows, studentRows, profileRows, attendanceRows, assessmentRows, ownershipRows, termResultRows, subjectResultRows,] = await Promise.all([
    ...
    const activeTerm = termRows
        ...
        ?? null;
    if (!activeTerm)
        throw new Error('No academic term is configured for this batch');
    const termOfferings = offeringRows.filter(row => row.termId === activeTerm.termId && row.status === 'active');
    if (termOfferings.length === 0)
        throw new Error('No active offerings exist for the selected batch term');
    ...
    const latestAttendanceByStudentOffering = latestRowByKey(attendanceRows.filter(row => relevantOfferingIds.has(row.offeringId)), row => `${row.studentId}::${row.offeringId}`);
    const latestTermResultByStudentId = latestRowByKey(termResultRows.filter(row => relevantStudentIds.has(row.studentId)), row => row.studentId);
    const subjectRowsByTranscriptId = new Map();
    ...
    const baseRunValues = {
        ...
        metricsJson: JSON.stringify({
            ...
            sectionDistribution: Object.fromEntries(Array.from(new Set(termOfferings.map(row => row.sectionCode))).map(sectionCode => [
                sectionCode,
    ...
    const teacherLoadRows = Array.from(loadsByFacultySemester.entries()).map(([key, rows]) => {
    ...
    const assessmentRowsByStudentOffering = new Map();
    assessmentRows
    ...
    for (const offering of termOfferings) {
        ...
        for (const enrollment of sectionEnrollments) {
            ...
            const ceContributions = [tt1Pct, tt2Pct, quizPct, assignmentPct].filter((value) => value != null);
