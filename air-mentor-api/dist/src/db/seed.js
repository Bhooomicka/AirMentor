...
import { adminRequestNotes, adminReminders, academicFaculties, academicTerms, academicCalendarAuditEvents, adminRequestTransitions, academicAssets, academicMeetings, academicTaskPlacements, academicTaskTransitions, academicTasks, adminRequests, academicRuntimeState, auditEvents, batches, alertDecisions, alertOutcomes, bridgeModules, branches, courseTopicPartitions, courses, curriculumEdges, curriculumImportVersions, curriculumNodes, curriculumCourses, departments, electiveBaskets, electiveOptions, electiveRecommendations, facultyAppointments, facultyCalendarAdminWorkspaces, facultyCalendarWorkspaces, courseOutcomeOverrides, facultyOfferingOwnerships, facultyProfiles, institutions, mentorAssignments, offeringAssessmentSchemes, offeringQuestionPapers, policyOverrides, reassessmentEvents, riskAssessments, roleGrants, sectionOfferings, semesterTransitionLogs, simulationResetSnapshots, simulationRuns, studentAssessmentScores, sessions, studentAgentCards, studentAgentMessages, studentAgentSessions, studentAcademicProfiles, studentAttendanceSnapshots, studentEnrollments, studentInterventions, studentLatentStates, studentObservedSemesterStates, students, teacherAllocations, teacherLoadProfiles, transcriptSubjectResults, transcriptTermResults, uiPreferences, userAccounts, userPasswordCredentials, } from './schema.js';
...
export async function seedIntoDatabase(db, client, now = nowIso()) {
    ...
    if (seedData.academicAssets) {
        ...
        if (runtimeEntries.length > 0) {
            ...
            const runtimePayload = seedData.academicAssets.runtime;
            const timetablePayload = runtimePayload.timetableByFacultyId ?? {};
            ...
            const adminWorkspacePayload = runtimePayload.adminCalendarByFacultyId ?? {};
