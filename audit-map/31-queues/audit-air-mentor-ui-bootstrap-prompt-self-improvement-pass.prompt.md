AirMentor audit OS pass: prompt-self-improvement-pass
...
# Prompt Self-Improvement Pass Prompt v2.0

Objective: improve prompt clarity, completeness, and output reliability using concrete failures, thin outputs, and audit-the-audit findings from completed passes.

Required outputs:

- prompt weaknesses linked to concrete pass failures or omissions
- exact edits required to prompts or templates
- prompt version history and change log updates
- evidence that the revised prompt closes a real omission or ambiguity

Rules:

- do not propose generic wording polish without a concrete forensic coverage reason
- prefer edits that make shallow completion harder and evidence requirements clearer
