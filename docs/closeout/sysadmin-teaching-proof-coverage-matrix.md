...
Current status as of `2026-03-30`: `DEF-02A-LIVE-GITHUB-PAGES-BANDS-DRIFT` and `DEF-05B-LIVE-A11Y-QUEUE-CONTRAST` are closed. Repo-local proof plus refreshed `LIVE-PROOF`, `LIVE-TEACHING`, `LIVE-ACCEPTANCE`, `LIVE-A11Y`, and `LIVE-KEYBOARD` are current on the deployed GitHub Pages + Railway stack.
...
| Hidden/archive restore flows | `SYSTEM_ADMIN` | overview + detail recycle actions | entity patch routes plus shared queue hide/restore wiring in `src/system-admin-action-queue.ts`, `src/system-admin-ui.tsx`, and `src/system-admin-live-app.tsx` | `npm test -- --run tests/system-admin-ui.test.tsx tests/system-admin-action-queue.test.ts tests/system-admin-accessibility-contracts.test.tsx` | `LIVE-ACCEPTANCE`, `LIVE-A11Y`, `LIVE-KEYBOARD` | acceptance report, accessibility report, keyboard report | 03A, 05B |
